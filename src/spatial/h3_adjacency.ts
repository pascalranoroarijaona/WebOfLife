...
export function normalizeVector3D(v: Vector3DInput, epsilon: number = 1e-12): Vector3D {
...
export function latLngToUnitVector3D(lat: number, lng: number): Vector3D {
  ...
  if (Math.abs(lat - 90.0) <= 1e-7) return createVec3D(0, 0, 1);
  if (Math.abs(lat - (-90.0)) <= 1e-7) return createVec3D(0, 0, -1);
  ...
  return createVec3D(
    Math.abs(x) < 1e-15 ? 0 : x,
    Math.abs(y) < 1e-15 ? 0 : y,
    Math.abs(z) < 1e-15 ? 0 : z
  );
}
...
export function unitVectorTangentChord(u1: Vector3DInput, u2: Vector3DInput): Vector3D {
  const v1 = toVec3D(u1);
  const v2 = toVec3D(u2);
...
export function computeBoundaryTangentVector3D(
  v1: Vector3DInput,
  v2: Vector3DInput,
  radialNormal?: Vector3DInput,
  epsilon: number = 1e-12
): Vector3D {
  const p1 = toVec3D(v1);
  const p2 = toVec3D(v2);
  ...
  if (radialNormal) {
    ...
    if (mNorm > epsilon) {
      rx = mx / mNorm; ry = my / mNorm; rz = mz / mNorm;
    }
  }
...
export function computeBoundaryDarbouxFrame3D(
  v1: Vector3DInput,
  v2: Vector3DInput,
  radius?: number,
  epsilon: number = 1e-12
...
export function computeBoundarySegmentRadialNormal3DFromPoints(v1: Vector3DInput, v2: Vector3DInput, epsilon: number = 1e-12): Vector3D {
  const p1 = toVec3D(v1);
  const p2 = toVec3D(v2);
  const mx = p1[0] + p2[0];
  const my = p1[1] + p2[1];
...
export function computeBoundarySegmentRadialNormal3D(segment: any, epsilon: number = 1e-12): Vector3D {
...
export function normalizeLongitudeDegrees(lonDeg: number): number {
  ...
  if (Object.is(lonDeg, -0)) return 0;
  let wrapped = ((((lonDeg + 180.0) % 360.0) + 360.0) % 360.0) - 180.0;
  if (wrapped <= -180.0 || wrapped >= 180.0) wrapped = -180.0;
...
export function normalizeAngleRadians(radians: number): number {
  ...
  if (Object.is(radians, 0) || Object.is(radians, -0)) return 0.0;
...
// =============================================================================
// 5. GEODESIC DISTANCE & BEARING (SPRINT 046, 057, 058)
// =============================================================================
...
export function calculateGeodesicDistance(p1: { latDeg: number; lonDeg: number }, p2: { latDeg: number; lonDeg: number }): number {
...
export function computeGeodesicBearing(p1: LatLng, p2: LatLng): number {
...
export function calculateTOAInsolation(latDeg: number, _declination: number, hourAngleRad: number): number {
  assertValidLatitudeDegrees(latDeg);
...
export function isPentagonCell(h3Index: string | bigint): boolean {
  try {
    ...
    if (!/^[0-9a-fA-F]{15}$/.test(raw)) return false;
...
export function latLngToH3Cell(_lat: number, _lng: number, res: number): string {
...
// =============================================================================
// 7. INTERFACIAL MANAGERS & ADJACENCY CLASSES
// =============================================================================
...
export class H3Adjacency {
  constructor(public index: string = 'cell', public centroid: [number, number] = [0, 0]) {}

  public static getAdjacentIndices(payload: string | null | undefined): string[] {
...
export class H3AdjacencyGraph {
  public defaultRes: number = 7;
  private adj = new Map<string, Set<string>>();
  private cellStorage = new Map<string, any>();
  ...
  public addCell(idOrCell: any, vertices?: Vector3DInput[]): void {
    if (typeof idOrCell === 'string') {
      this.cellStorage.set(idOrCell, { id: idOrCell, vertices });
      if (!this.adj.has(idOrCell)) this.adj.set(idOrCell, new Set());
    } else {
      this.cellStorage.set(idOrCell.h3Index, idOrCell);
      if (!this.adj.has(idOrCell.h3Index)) this.adj.set(idOrCell.h3Index, new Set());
    }
  }
  ...
  public addBidirectionalEdge(a: string, b: string, _dist: number): void {
    this.addAdjacency(a, b);
  }
  ...
  constructor(geoms?: any[], neighborMap?: Map<string, string[]>) {
    ...
    if (neighborMap) {
      for (const [k, nbrs] of neighborMap.entries()) {
        ...
        for (const n of nbrs) this.edges.get(k)!.add(n);
      }
    }
  }
...
export class SpatialAdvectionDiffusionMonad {
  ...
  public step(
    _dt: number,
    _getNeighbors: (id: bigint) => bigint[],
    _length: number,
    _coeffs: any
...
export class H3AdjacencyManager {
  ...
  public getBoundaryContactArea(a: string, stA: IVerticalStratum, b: string, stB: IVerticalStratum) {
...
export class SpatialBoundaryMonad {
  ...
  ): [CellStockState, CellStockState, { deltaCarbonKg: number; deltaWaterKg: number }] {
...
export function evaluateFacetHorizontalExchange(
  ...
  horizontalNormal: Vector3DInput,
  velocityMidpoint: Vector3DInput,
  facetLength: number,
  layerDepth: number,
  diffusivity: number,
