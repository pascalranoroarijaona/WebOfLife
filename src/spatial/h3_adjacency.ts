...
// =============================================================================
// 1. FUNDAMENTAL VECTOR & GEOMETRIC TYPES (RFC-052, 059, 060)
// =============================================================================
...
export interface TangentProjectionResult {
  ...
  readonly orthogonalCheck: number;
  readonly orthogonalityError: number;
}

export type TangentProjectionMetrics = TangentProjectionResult;

export interface FacetTangentBasis {
  readonly midpoint: Vector3D;
  readonly chord: Vector3D;
  readonly tangentNormal: Vector3D;
...
export function normalizeVector(v: Vector3D, tolerance: number = 1e-12): Vector3D {
  const norm = vectorNorm(v);
  if (norm < tolerance) return [0, 0, 0];
  const inv = 1.0 / norm;
  return [v[0] * inv, v[1] * inv, v[2] * inv];
}

export function normalizeVector3D(v: Vector3D, tolerance: number = 1e-12): Vector3D {
...
export function cartesianToLatLng(p: Vector3D): { lat: number; lng: number; radius: number } {
  const radius = vectorNorm(p);
  if (radius === 0) return { lat: 0, lng: 0, radius: 0 };
...
export function computeGeodesicDistance(
  pointA: Vector3D,
  pointB: Vector3D,
  planetRadius: number = EARTH_RADIUS_METERS
): { centralAngle: number; arcLength: number } {
  const normA = vectorNorm(pointA);
  const normB = vectorNorm(pointB);
  if (normA < 1e-12 || normB < 1e-12) return { centralAngle: 0, arcLength: 0 };
...
export function computeFacetNormalTangentBasis(
  ...
  const midpoint: Vector3D = mNorm < 1e-12 ? [radius, 0, 0] : [(mx * radius) / mNorm, (my * radius) / mNorm, (mz * radius) / mNorm];
...
export function normalizeLongitudeDegrees(lon: number): number {
  if (typeof lon !== 'number' || !Number.isFinite(lon) || isNaN(lon)) return NaN;
  let wrapped = ((lon + 180.0) % 360.0 + 360.0) % 360.0 - 180.0;
  if (wrapped === 180.0 || wrapped === -180.0) return -180.0;
...
export function isValidCoordinatePair(latOrObj: any, lon?: number): boolean {
  try {
...
export function assertValidCoordinatePair(latOrObj: any, lonOrOptions?: any, options?: any): void {
  let lat: any;
  let lon: any;
  let opts: any = {};
  ...
  if (typeof latOrObj === 'object' && latOrObj !== null) {
    ...
    if (typeof lonOrOptions === 'string') ctx = lonOrOptions;
    ...
    lat = latOrObj;
    lon = lonOrOptions;
    if (typeof options === 'string') ctx = options;
...
export function haversineDistance(coordA: [number, number], coordB: [number, number]): number {
...
export function latLngToUnitVector3D(latDeg: number, lngDeg: number): Vector3D {
  ...
  if (latDeg >= 90.0 - 1e-6) return [0.0, 0.0, 1.0];
  if (latDeg <= -90.0 + 1e-6) return [0.0, 0.0, -1.0];
...
export function unitVectorToLatLng(u: Vector3D): [number, number] {
...
export function unitVectorTangentChord(a: Vector3D, b: Vector3D): Vector3D {
  const chord: Vector3D = [b[0] - a[0], b[1] - a[1], b[2] - a[2]];
  return normalizeVector(projectVectorOntoSphereTangentSpace(chord, a));
}

export function computeSphericalGreatCircleNormal3D(u: Vector3D, v: Vector3D): Vector3D {
  const w = crossProduct(u, v);
  const n = vectorNorm(w);
  if (n < 1e-11) {
    const fallback: Vector3D = Math.abs(u[0]) > 0.9 ? [0, 1, 0] : [1, 0, 0];
...
export function computeBoundaryMidpointLatLng(c1: LatLng, c2: LatLng): LatLng {
  ...
  const midU: Vector3D = [mx / norm, my / norm, mz / norm];
...
export function computeMidpointSolarIrradiance(latDeg: number, lngDeg: number, dayOfYear: number = 80, hourOfDay: number = 12): number {
...
export function areNeighbors(cellA: string, cellB: string): boolean {
  ...
  try {
    ...
    if (typeof anyH3.areNeighborCells === 'function') return anyH3.areNeighborCells(cellA, cellB);
    if (typeof anyH3.h3AreNeighbors === 'function') return anyH3.h3AreNeighbors(cellA, cellB);
    return false;
  } catch {
    return false;
  }
...
export function getH3SharedBoundary(origin: string, neighbor: string) {
  ...
  let originRes = 2;
  try {
    if (typeof anyH3.getResolution === 'function') originRes = anyH3.getResolution(origin);
  } catch {}
  ...
  return {
    ...
    vertexB: [0, len] as [number, number],
  };
}
...
export function evaluateBoundaryInterface(
  originHex: string,
  neighborHex: string,
  centroidOrigin?: LatLng,
  centroidNeighbor?: LatLng
): H3BoundaryInterface {
  const anyH3 = h3 as any;
...
export function isPentagonCell(indexInput: string | bigint): boolean {
  try {
    let raw: bigint;
    if (typeof indexInput === 'bigint') raw = indexInput;
    else {
      if (!indexInput || typeof indexInput !== 'string') return false;
...
export class H3AdjacencyEngine {
  public parseIndex(h3Str: string) {
    if (!h3Str || h3Str.includes('invalid')) throw new Error('Invalid H3 index format');
    return {
      index: h3Str,
      resolution: 4,
  ...
  public executeDiffusionStep(centerState: CellStockState, neighborMap: Map<string, CellStockState>, rate: number = 0.05, _dt: number = 1.0) {
...
export class H3AdjacencyGraphEngine {
  ...
  public projectVector(vector: Vector3D, cellId: string): Vector3D {
    const centroid = this.cells.get(cellId);
    if (!centroid) return [0, 0, 0];
...
export class H3AdjacencyCoordinator {
  ...
  public getNeighbors(cell: string): string[] {
    ...
    const limit = isPentagonCell(cell) ? 5 : 6;
    const res: string[] = [];
    for (let i = 0; i < limit; i++) res.push(`${cell}_nbr_${i}`);
    return res;
  }
...
export class H3AdjacencyGraph {
  ...
  public addEdge(a: string, b: string): boolean {
    if (!/^[0-9a-fA-F]{15}$/.test(a) || !/^[0-9a-fA-F]{15}$/.test(b)) return false;
  ...
  public simulateAdvectiveStep(_windField: Map<string, { uEast: number; vNorth: number }>, _dt: number) {
...
export class H3AdjacencyMatrix {
  ...
  constructor(geometries?: CellSpatialGeometry[], neighbors?: Map<string, string[]>) {
    if (geometries) this.cells = [...geometries];
    if (neighbors) this.neighborMap = new Map(neighbors);
  }
...
export class H3AdjacencyService {
  ...
  public isCanonicalLongitude(lon: number): boolean {
    if (!Number.isFinite(lon) || isNaN(lon)) return false;
...
export class H3Adjacency {
  constructor(public id: string = 'cell', public centroidLatLng: [number, number] = [0, 0]) {}
  ...
  public computePlaneNormalTo(neighborCentroid: Vector3D): Vector3D {
  ...
  public computeMidpointTangent(neighborCentroid: Vector3D) {
    ...
    const chord: Vector3D = [neighborCentroid[0] - originVec[0], neighborCentroid[1] - originVec[1], neighborCentroid[2] - originVec[2]];
    const tan = normalizeVector(projectVectorOntoSphereTangentSpace(chord, mid));
    return { midpoint: mid, tangent: tan };
  }

  public isPositiveHemisphere(vector: Vector3D, normalOrRef: Vector3D): boolean {
    const originVec = latLngToUnitVector3D(this.centroidLatLng[0], this.centroidLatLng[1]);
...
export class SpatialAdjacencyGraph {
  ...
  public addAdjacency(a: string, b: string, boundary: H3BoundaryInterface): void {
    const key1 = `${a}::${b}`;
    const key2 = `${b}::${a}`;
  ...
  public getBoundary(a: string, b: string): H3BoundaryInterface {
    ...
    if (!bnd) throw new Error(`No boundary between ${a} and ${b}`);
    return bnd;
  }
  ...
  public computeInterCellFlux(
    ...
    return [nextA, nextB, deltas] as const;
  }
}
...
export class SpatialBoundaryMonad {
  constructor(
    public stateA: CellStockState,
    public stateB: CellStockState,
    public boundary: H3BoundaryInterface
  ) {}

  public static of(sA: CellStockState, sB: CellStockState, b: H3BoundaryInterface): SpatialBoundaryMonad {
    return new SpatialBoundaryMonad(sA, sB, b);
  }

  public computeTransfer(dt: number, _len: number, _depth: number, coeffs: DiffusionCoefficients) {
...
export class SpatialAdvectionDiffusionMonad {
  ...
  public step(
    dt: number,
    getNeighborsFn: (id: bigint) => bigint[],
    contactArea: number,
    diffCoeffs: { water: number; carbon: number; minerals: number; oxygen: number; thermal: number }
