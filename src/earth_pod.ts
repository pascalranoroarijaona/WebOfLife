...
export class CyclePOD extends ThermodynamicStructure {
  ...
  computeReservoirImbalance(): number {
    const totalTransfer = Object.values(this.transferRates).reduce(
      (a, b) => a + b,
      0
    );
...
export const CARBON_CYCLE = new CyclePOD(
  "Carbon Cycle",
  {
    atmosphere: 850,
    ocean_surface: 900,
...
export const NITROGEN_CYCLE = new CyclePOD(
  "Nitrogen Cycle",
  {
...
export const PHOSPHORUS_CYCLE = new CyclePOD(
  "Phosphorus Cycle",
  {
...
export const WATER_CYCLE = new CyclePOD(
  "Water Cycle",
  {
    ocean: 1_338_000_000,
    atmosphere: 12900,
...
export class SpherePOD extends ThermodynamicStructure {
  ...
  importFreeEnergy(tick: number): number {
    ...
    const sum = this.involvedCycles.reduce(
      (s, c) => s + c.importFreeEnergy(tick),
      0
    );
  ...
  maintainFarFromEquilibrium(tick: number): EntropyState {
    const states = this.involvedCycles.map((c) =>
      c.maintainFarFromEquilibrium(tick)
    );
...
export class BiomePOD extends ThermodynamicStructure {
  ...
  maintainFarFromEquilibrium(_tick: number): EntropyState {
    const biodiversityIndex =
      this.species.length / Math.max(1, this.tickCreated + 1);
...
export class GeoBiomePOD extends BiomePOD {
  constructor(
    ...
    sphere: SpherePOD,
    areaKm2: number,
    public boundingPolygon: GeoLocation[]
  ) {
...
export class SpeciesPOD extends ThermodynamicStructure {
  ...
  executeMethod(methodName: string, target?: ThermodynamicStructure): string {
    ...
    return `${this.name}.${methodName}() executed on ${
  ...
  static applyPredatorPreyStep(
    ...
    predator.population = Math.max(
      2,
      predPop + conversionEfficiency * preyPop * predPop - deathRate * predPop
    );
...
export class EarthPOD extends ThermodynamicStructure {
  ...
  maintainFarFromEquilibrium(tick: number): EntropyState {
    const cycleStates = this.cycles.map((c) =>
      c.maintainFarFromEquilibrium(tick)
    );
    const degradingCount = cycleStates.filter(
      (s) => s === EntropyState.DEGRADING
    ).length;
  ...
  globalEntropyReport() {
    return {
      ...
      cycles: Object.fromEntries(
        this.cycles.map((c) => [c.name, c.entropyState])
      ),
      spheres: Object.fromEntries(
        this.spheres.map((s) => [s.name, s.entropyState])
      ),
...
export function bootstrapMegaPod() {
  ...
  const temperateForest = new GeoBiomePOD(
    "Temperate Forest",
    BIOSPHERE,
    1_000_000,
  ...
  const fauna = new SpeciesPOD(
    "Fauna",
    "Heterotrophic assemblage",
    2,
    temperateForest,
    35
  );
  const fungi = new SpeciesPOD(
    "Fungi",
