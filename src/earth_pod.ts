...
export abstract class ThermodynamicStructure {
  public id: string;
  public name: string;
  ...
  computeGibbsFreeEnergy(deltaH: number, tempK: number, deltaS: number): number {
    return deltaH - tempK * deltaS;
  }

  // 2. Entropie de Configuration de Shannon
  computeShannonEntropy(): number {
    let total = 0;
...
export class SpherePOD extends ThermodynamicStructure {
  constructor(name: string, public involvedCycles: CyclePOD[]) { super(name); }
  ...
  importFreeEnergy(tick: number): number {
    ...
    return sum / this.involvedCycles.length;
  }
...
export class BiomePOD extends ThermodynamicStructure {
  ...
  public jacobianStabilityIndex: number = 0.85;

  constructor(name: string, public sphere: SpherePOD, public areaKm2: number) { super(name); }

  importFreeEnergy(tick: number): number {
    return this.species.reduce((s, sp) => s + sp.importFreeEnergy(tick), 0);
  }

  exportEntropy(tick: number): number {
    return this.species.reduce((s, sp) => s + sp.exportEntropy(tick), 0);
  }

  maintainFarFromEquilibrium(_tick: number): EntropyState {
    const biodiversityIndex = this.species.length / Math.max(1, this.tickCreated + 1);
    if (biodiversityIndex < 0.1 || this.jacobianStabilityIndex < 0.3) return EntropyState.DEGRADING;
    return EntropyState.STEADY;
  }
...
export class SpeciesPOD extends ThermodynamicStructure {
  ...
  importFreeEnergy(_tick: number): number {
    if (this.trophicLevel === 1) return this.population * 0.01;
    return this.population * this.netFlow("biomass") * 0.1;
  }

  exportEntropy(_tick: number): number {
    return this.population * 0.008;
  }
...
// 7, 8, 9, 10. Attributs étendus de la Monade Humaine
export interface IndividualMonad {
  monadId: string;
  wellbeingUnits: number;
  identityHash: string;
  resourceExtractionLog: Flow[];
  healthTelemetry: { autonomicRecovery: number; metabolicEfficiency: number; };
  safetyConstraints: { thermalComfortMargin: number; basicResourceAccess: boolean; };
  behavioralVectors: { materialFootprintTons: number; industrialConsumptionRate: number; };
  justiceAndVeto: { ontologicalVetoActive: boolean; taprootScriptHash: string; };
  digitalMaterialPassport: { verifiedProvenance: boolean; carbonTokenId: string; };
}

export function createIndividualMonad(
  monadId: string,
  wellbeingUnits: number,
  identityHash: string
): IndividualMonad {
...
export class EarthPOD extends ThermodynamicStructure {
  ...
  static getInstance(): EarthPOD {
    if (!EarthPOD._instance) { EarthPOD._instance = new EarthPOD(); }
    return EarthPOD._instance;
  }

  importFreeEnergy(_tick: number): number { return this.solarInputWatts; }
  exportEntropy(_tick: number): number { return this.solarInputWatts * 0.997; }
  ...
  fullTick(tickNum: number) {
    ...
    for (const cycle of this.cycles) { result.cycles[cycle.name] = cycle.tick(tickNum); }
    for (const biome of this.biomes) {
      result.biomes[biome.name] = biome.tick(tickNum);
      for (const species of biome.species) { species.tick(tickNum); }
    }
    return result;
  }
...
export class StellarMonad extends ThermodynamicStructure {
  ...
  importFreeEnergy(_tick: number): number { return 0; }
  exportEntropy(_tick: number): number { return 3.846e26; }
  maintainFarFromEquilibrium(_tick: number): EntropyState {
