...
INITIAL_AI_STUDIO_CREDITS = 36.00  # Valeur de référence initiale en USD
...
def wipe_all_sprint_data() -> None:
    ...
    # 2. Wipe docs/sprints/ folders completely
    sprints_parent = REPO_ROOT / "docs" / "sprints" 
    if sprints_parent.exists():
        for item in sprints_parent.iterdir():
            if item.is_dir() and item.name.startswith("sprint_"):
                shutil.rmtree(item, ignore_errors=True)
                print(f"    🧹 Removed sprint folder: docs/sprints/{item.name}")

    # 3. Clean dist/src/ and restore baseline JS files from original backups
    dist_parent = REPO_ROOT / "dist" / "src"
    if dist_parent.exists():
        for item in dist_parent.iterdir():
            if item.is_file():
                if item.name not in ["main.js", "earth_pod.js", "main.original.js", "earth_pod.original.js"]:
                    item.unlink()
                    print(f"    🧹 Removed generated build file: dist/src/{item.name}")
            elif item.is_dir():
                shutil.rmtree(item, ignore_errors=True)
                print(f"    🧹 Removed generated build subdirectory: dist/src/{item.name}/")

        # Restore main.js and earth_pod.js from pristine original JS builds if available
        main_js_orig = dist_parent / "main.original.js"
        main_js = dist_parent / "main.js"
        if main_js_orig.exists():
            shutil.copy(main_js_orig, main_js)
            print("    🔄 Restored dist/src/main.js from dist/src/main.original.js")

        earth_js_orig = dist_parent / "earth_pod.original.js"
        earth_js = dist_parent / "earth_js.js" if False else dist_parent / "earth_pod.js"
        if earth_js_orig.exists():
            shutil.copy(earth_js_orig, earth_js)
            print("    🔄 Restored dist/src/earth_pod.js from dist/src/earth_pod.original.js")
    ...
    # 5. Clean src/ and restore from pristine .original.ts backups
    src_parent = REPO_ROOT / "src"
    if src_parent.exists():
        for item in src_parent.iterdir():
            if item.is_file():
                if item.name not in ["main.ts", "earth_pod.ts", "main.original.ts", "earth_pod.original.ts"]:
                    item.unlink()
        ...
        # Restore main.ts and earth_pod.ts from pristine originals if available
        main_orig = src_parent / "main.original.ts"
        main_ts = src_parent / "main.ts"
        if main_orig.exists():
            shutil.copy(main_orig, main_ts)
            print("    🔄 Restored src/main.ts from src/main.original.ts")

        earth_orig = src_parent / "earth_pod.original.ts"
        earth_ts = src_parent / "earth_pod.ts"
        if earth_orig.exists():
            shutil.copy(earth_orig, earth_ts)
            print("    🔄 Restored src/earth_pod.ts from src/earth_pod.original.ts")
    ...
    # 9. Remove BACKLOG.md, index.html and any .mp3 files from docs/
    backlog_file = REPO_ROOT / "docs" / "BACKLOG.md"
    if backlog_file.exists():
        backlog_file.unlink()
        print("    🧹 Removed docs/BACKLOG.md")

    indexdocs_file = REPO_ROOT / "docs" / "index.html"
    if indexdocs_file.exists():
        indexdocs_file.unlink()
        print("    🧹 Removed docs/index.html")

    docs_parent = REPO_ROOT / "docs"
    if docs_parent.exists():
        for mp3_file in docs_parent.glob("**/*.mp3"):
            mp3_file.unlink()
            print(f"    🧹 Removed audio artifact: {mp3_file.relative_to(REPO_ROOT)}")

    # 10. Clear dist/tests/ directory
    dist_tests_parent = REPO_ROOT / "dist" / "tests"
    if dist_tests_parent.exists():
        shutil.rmtree(dist_tests_parent, ignore_errors=True)
        dist_tests_parent.mkdir(parents=True, exist_ok=True)
        print("    🧹 Cleared dist/tests/ directory.")

    # 11. Clean all files from .agent_logs/ (preserving directory structure)
    logs_parent = REPO_ROOT / ".agent_logs"
    if logs_parent.exists():
        for log_item in logs_parent.iterdir():
            if log_item.is_file():
                log_item.unlink()
                print(f"    🧹 Removed log file: .agent_logs/{log_item.name}")
            elif log_item.is_dir():
                shutil.rmtree(log_item, ignore_errors=True)
                print(f"    🧹 Removed log subdirectory: .agent_logs/{log_item.name}/")
        print("    🧹 Cleaned all files from .agent_logs/.")
    ensure_tsconfig()
