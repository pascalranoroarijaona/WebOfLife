...
export class TemperatureNormalizationEngine {
    ...
    toCelsius(value, unit = 'C') {
        const rawK = unit === 'C' ? value + THERMODYNAMIC_CONSTANTS.ZERO_CELSIUS_IN_KELVIN : value;
        const clampedK = Math.max(THERMODYNAMIC_CONSTANTS.PLANETARY_TEMP_MIN_K, Math.min(THERMODYNAMIC_CONSTANTS.PLANETARY_TEMP_MAX_K, rawK));
