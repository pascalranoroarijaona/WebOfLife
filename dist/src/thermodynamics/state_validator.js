...
export class DetritivoreMonad {
    static scavenge(carcass, patch, ledger) {
        const assimilated = new ElementalStocks(carcass.carbon * 0.15, carcass.nitrogen * 0.15, carcass.phosphorus * 0.15, carcass.water * 0.15, carcass.oxygen * 0.15, carcass.energy * 0.15);
...
export class StateValidator {
    toleranceThreshold;
    conservationHook;
    constructor(toleranceThreshold = 1e-6) {
        this.toleranceThreshold = toleranceThreshold;
    }
    evaluateDiscrepancy(previousState, currentState, integratedFluxes, tolerance) {
        const activeTolerance = tolerance ?? this.toleranceThreshold;
        const poolDiscrepancies = {};
        const prevPools = previousState?.pools ?? previousState?.stocks ?? {};
        const currPools = currentState?.pools ?? currentState?.stocks ?? {};
        ...
        const timestampVal = currentState?.timestamp ?? currentState?.tick ?? Date.now();
        return {
    ...
    assertValidState(vector) {
        ...
        if (!res.isValid) {
            throw new Error(`State validation failed: ${res.errors?.map((e) => e.reason).join(', ') ?? 'Unknown violation'}`);
        }
    ...
    validateConservation(previousState, currentState, fluxes, dt = 1.0) {
    ...
    validateStockConservation(previousState, currentState, fluxes, dt = 1.0) {
        const prevStocks = previousState?.stocks instanceof Map ? Object.fromEntries(previousState.stocks) : (previousState?.stocks ?? {});
        const currStocks = currentState?.stocks instanceof Map ? Object.fromEntries(currentState.stocks) : (currentState?.stocks ?? {});
        const discrepancies = new Map();
        let valid = true;
    ...
    assertConservation(previousState, currentState, fluxes, dt = 1.0) {
    ...
    calculateExpectedDelta(vector, dt) {
        ...
        let totalInflow = 0;
        for (const val of inflows.values())
            totalInflow += Number(val) || 0;
        let totalOutflow = 0;
        for (const val of outflows.values())
    ...
    static validateConservation(prevVector, nextVector, fluxes, dt, tolerance = 1e-9) {
...
export function executeThermodynamicTransition(state, transformFn) {
    try {
