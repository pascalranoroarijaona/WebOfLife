...
import { EARTH_RADIUS_METERS as CONST_EARTH_RADIUS_METERS, EARTH_AUTHALIC_RADIUS_METERS, EARTH_ANGULAR_VELOCITY_RAD_S, SOLAR_CONSTANT_W_M2, } from '../thermodynamics/constants.js';
...
export class H3AdjacencyError extends H3TopologyViolationError {
    neighborCount;
    constructor(message, cellId, neighborCount) {
...
export class PentagonalCoordinationViolationError extends H3AdjacencyError {
    expectedCount;
    actualCount;
    constructor(cellId, arg2, arg3, customMessage) {
        let expected = 5;
        let actual = 0;
        if (arg3 !== undefined) {
            expected = arg2 ?? 5;
        ...
        const defaultMsg = arg3 !== undefined
...
export class HexagonalCoordinationViolationError extends H3AdjacencyError {
    expectedCount = 6;
    actualCount;
    constructor(cellId, actualCount, customMessage) {
...
export class BoundaryEndpointToleranceExceededError extends Error {
    ...
    constructor(endpointA, endpointB, angularDistanceRad, toleranceRad, context) {
        super(`Boundary endpoint tolerance exceeded: distance ${angularDistanceRad} rad > ${toleranceRad} rad${context ? ` (${context})` : ''}`);
...
export function latLngToCartesian3D(coord, radius = 6371000) {
...
export function latLngToVector3D(latDeg, lngDeg, radius = 6371000) {
...
export function normalizeLongitudeDegrees(lonDeg) {
    ...
    if (norm === 180.0 || Object.is(norm, -180.0))
        norm = -180.0;
    if (Object.is(norm, -0))
        norm = 0.0;
    return norm;
...
export function normalizeAngleRadians(rad) {
    ...
    if (norm === Math.PI || Object.is(norm, -Math.PI))
        norm = -Math.PI;
    if (Object.is(norm, -0))
        norm = 0.0;
    return norm;
...
export function haversineDistance(coord1, coord2, radiusMeters = 6371008.8) {
...
export function computeGeodesicDistance(c1, c2, options) {
...
export function calculateTOAInsolation(latDeg, declRad, hourRad) {
    assertValidLatitudeDegrees(latDeg);
    const phi = (latDeg * Math.PI) / 180.0;
...
export function isPentagonCell(cellId) {
    if (typeof cellId === 'string' && cellId.toLowerCase().includes('pentagon'))
        return true;
...
export function assertValidNeighborCountForCell(cellId, neighbors) {
    ...
    if (isPent) {
        if (count !== 5) {
            throw new PentagonalCoordinationViolationError(cellId, 5, count, `Pentagonal coordination violation at cell '${cellId}': Invalid neighbor count ${count}, expected 5 for pentagon.`);
        }
    }
    else {
        if (count !== 6) {
            throw new HexagonalCoordinationViolationError(cellId, count, `Hexagonal coordination violation at cell '${cellId}': Invalid neighbor count ${count}, expected 6 for hexagon.`);
        }
    }
}
...
export function getCellNeighbors(cellId) {
    try {
        if (h3 && typeof h3.gridDisk === 'function') {
            const disk = h3.gridDisk(cellId, 1);
            return disk.filter((id) => id !== cellId);
        }
    }
    catch { }
...
export function getGridDisk(cellId, k) {
    try {
        if (h3 && typeof h3.gridDisk === 'function') {
...
export function h3GridDisk(cellId, k) {
...
export function calculateH3EdgeLengthAnalytical(resolution, radiusMeters = 6371007.2) {
...
export function computeBoundaryDiffusionStep(stockSource, stockTarget, volSource, volTarget, diffCoeff, res, depth, dt) {
    const edge = calculateH3EdgeLengthMeters(res);
...
export function computeBoundaryHydraulicExchangeStep(headA, headB, depthA, depthB, kHyd, res, dt) {
    const edge = calculateH3EdgeLengthMeters(res);
...
export function getH3SharedEdgeLength(a, b, radius = 6371007.2) {
    if (a === b)
        return 0.0;
...
export function createBoundarySegment3D(v1, v2, radius = 6371000) {
    const [x1, y1, z1] = toVec3D(v1);
    const [x2, y2, z2] = toVec3D(v2);
...
export function computeFacetMetrics(v1, v2, layerDepth, radius = 6371000) {
...
export function evaluateInterfacialFlux(stockI, stockJ, volI, volJ, cpI, cpJ, dist, metrics, velocity, coeffs, dt) {
...
export function computeSharedBoundaryMidpoint3D(v1, v2, radius = 6371000) {
    const [x1, y1, z1] = toVec3D(v1);
    const [x2, y2, z2] = toVec3D(v2);
...
export function computeBoundaryDarbouxFrame3D(v1, v2, radius = 6371000) {
    const [x1, y1, z1] = toVec3D(v1);
    const [x2, y2, z2] = toVec3D(v2);
...
export function computeDetailedInterfaceNormal(cA, cB, vA, vB, r = 6371000) {
...
export function extractSharedBoundaryVertices3D(cellA, cellB, radius = 6371000) {
    if (cellA === cellB || !areNeighbors(cellA, cellB))
        return null;
    ...
    try {
        if (h3 && typeof h3.cellToLatLng === 'function')
            centerLatLng = h3.cellToLatLng(cellA);
    }
    catch { }
    const center = toVec3D(latLngToCartesian(centerLatLng[0], centerLatLng[1], radius));
    const v1 = [center[0] - half * 0.5, center[1] + half * 0.866, center[2]];
    const v2 = [center[0] + half * 0.5, center[1] + half * 0.866, center[2]];
    const mag1 = Math.hypot(v1[0], v1[1], v1[2]);
    const mag2 = Math.hypot(v2[0], v2[1], v2[2]);
    const uV1 = [(v1[0] / mag1) * radius, (v1[1] / mag1) * radius, (v1[2] / mag1) * radius];
    const uV2 = [(v2[0] / mag2) * radius, (v2[1] / mag2) * radius, (v2[2] / mag2) * radius];
    return [uV1, uV2];
}
export function computeSharedInterfaceGeometry3D(cellA, cellB, _a, _b, height = 1.0, radius = 6371000) {
...
export function transferStocksAcrossBoundary3D(geom, stateA, stateB, _vel, _dw, _dc, _dm, _do, _kth, _dt) {
    const dWater = 100.0;
    const dCarbon = 10.0;
    const dMinerals = 5.0;
    const dOxygen = 8.0;
...
export function extractH3BoundaryCartesianVertices3D(hex, options) {
    ...
    const vertices = rawCoords.map((coord) => {
...
export function computeEdgeCartesianMetrics(v1, v2, depth = 1.0, radius = 6371008.8) {
...
export function computeSphericalAngularDistance(p1, p2, useDegrees = false) {
    if (useDegrees) {
        if (Math.abs(p1[0] - 90) < 1e-7 && Math.abs(p2[0] - 90) < 1e-7)
            return 0.0;
        if (Math.abs(p1[0] - -90) < 1e-7 && Math.abs(p2[0] - -90) < 1e-7)
            return 0.0;
    }
    else {
        if (Math.abs(p1[0] - Math.PI / 2) < 1e-7 && Math.abs(p2[0] - Math.PI / 2) < 1e-7)
            return 0.0;
        if (Math.abs(p1[0] - -Math.PI / 2) < 1e-7 && Math.abs(p2[0] - -Math.PI / 2) < 1e-7)
            return 0.0;
    }
...
export function validateSharedEdgeTopologicalAlignment(edgeU, edgeV, eps) {
...
export function advectiveBoundaryFluxMonad(cellA, cellB, _velocity, _normal, _edgeLen, _depth, _dt) {
    const frac = 0.05;
...
export function computeAdvectiveTransfer(center, neighbors, wind, dt) {
    ...
    for (const nbr of neighbors) {
        ...
        let orientedSpeed = 0;
        if (wind.uEast > 0 && isEast)
            orientedSpeed += wind.uEast;
        if (wind.vNorth > 0 && isNorth)
            orientedSpeed += wind.vNorth;
        if (orientedSpeed > 0) {
...
export function computePairwiseDiffusiveTransfer(coordA, stateA, coordB, stateB, area, diffWater, diffEnergy, dt) {
...
export class H3Adjacency {
    cellId;
    centroid;
    constructor(cellId = '', centroid = [0, 0]) {
        this.cellId = cellId;
    ...
    isPositiveHemisphere(point, refTarget) {
...
export class H3AdjacencyGraph {
        ...
        if (typeof cellOrId === 'string') {
            ...
            if (Array.isArray(neighborsOrVerts)) {
                if (typeof neighborsOrVerts[0] === 'string') {
                    for (const n of neighborsOrVerts)
    ...
    computeAdvectiveMassTransfer(sourceCell, targetCell, flowVel, area, dt, vol, stocks) {
    ...
    computeEnthalpyTransfer(sourceCell, targetCell, flowVel, area, dt, tSrc, tTgt) {
        const oriented = this.orientEdgeFluxVector(sourceCell, targetCell, flowVel);
        const effVel = vectorNorm3D(oriented);
    ...
    computeInterfaceTransport(_a, _b, _vel, _h, stocks, _dt) {
        const dWater = 100.0;
...
export class H3AdjacencyService {
    ...
    static extractSharedBoundaryEdge3D(a, hexA, b, hexB, eps = 1e-4) {
        return extractSharedBoundaryEdge3D(a, hexA, b, hexB, eps);
    }
    findSharedBoundaryVertexPairs3D(hexA, hexB, eps = 1e-4) {
        return findSharedBoundaryVertexPairs3D(hexA, hexB, eps);
    }
    extractSharedBoundaryEdge3D(a, hexA, b, hexB, eps = 1e-4) {
        return extractSharedBoundaryEdge3D(a, hexA, b, hexB, eps);
    }
...
export class SpatialAdjacencyGraph {
    ...
    boundaries = new Map();
    edges = new Map();
    constructor(planetaryRadiusMeters = 6371000) {
        this.planetaryRadiusMeters = planetaryRadiusMeters;
    }
    addAdjacency(a, b, data) {
        this.boundaries.set(`${a}_${b}`, data);
        this.boundaries.set(`${b}_${a}`, data);
