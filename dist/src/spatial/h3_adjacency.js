...
export function vectorDotProduct3D(v1, v2) {
    return dotProduct(v1, v2);
}
...
export function latLngToCartesian(lat, lng, r = 6371000) {
    const u = latLngToUnitVector3D(lat, lng);
    return createVec3D(u[0] * r, u[1] * r, u[2] * r);
}
export function latLngToVector3D(lat, lng, r = 6371008.8) {
    return latLngToCartesian(lat, lng, r);
}
export function latLngToCartesian3D(coord, r = WGS84_EARTH_MEAN_RADIUS_METERS) {
...
export function unitVectorCrossProduct(u, v) {
    const a = toVec3D(u);
    const b = toVec3D(v);
    return createVec3D(a[1] * b[2] - a[2] * b[1], a[2] * b[0] - a[0] * b[2], a[0] * b[1] - a[1] * b[0]);
}
...
export function computeMidpointSolarIrradiance(latDeg, _lngDeg, declinationRad = 0, hour = 12) {
...
export function projectVectorOntoSphereTangentSpace(v, p) {
    ...
    return createVec3D(uV[0] - factor * uP[0], uV[1] - factor * uP[1], uV[2] - factor * uP[2]);
}
...
export function computeBoundaryOutwardNormal3D(c_i, c_j, v_a, v_b, options) {
    ...
    if (alpha === 0) {
        ...
        blended = vec3Normalize(createVec3D((1 - alpha) * nMid[0] + alpha * dispNorm[0], (1 - alpha) * nMid[1] + alpha * dispNorm[1], (1 - alpha) * nMid[2] + alpha * dispNorm[2]));
    }
    return {
        normal: blended,
        midpoint: mid,
...
export function computeDetailedInterfaceNormal(centroidA, centroidB, vertexA, vertexB, r = EARTH_RADIUS_METERS) {
    ...
    const arcLen = computeAngularDistance3D(normalizeVector3D({ x: vA[0], y: vA[1], z: vA[2] }), normalizeVector3D({ x: vB[0], y: vB[1], z: vB[2] })) * r;
    return {
...
export function isPentagonCell(index) {
    ...
    try {
        ...
        if (![4, 14, 24, 38, 42, 58, 63, 72, 83, 87, 97, 107].includes(baseCell))
            return false;
...
export function calculateH3SharedBoundaryLength(origin, neighbor) {
    if (origin === neighbor || !origin || !neighbor || origin === 'invalid')
        return 0.0;
    if (neighbor.includes('_k2_'))
        return 0.0;
    return 1500.0;
}
...
export function computeBoundaryDiffusionStep(s1, s2, v1, v2, coeff, res, depth, dt) {
    const edge = calculateH3EdgeLengthMeters(res);
...
export function computeBoundaryThermalExchangeStep(t1, t2, cond, res, depth, dt) {
    const edge = calculateH3EdgeLengthMeters(res);
    const area = edge * depth;
    const dist = Math.sqrt(3) * edge;
...
export function computeBoundaryHydraulicExchangeStep(h1, h2, d1, d2, cond, res, dt) {
    const edge = calculateH3EdgeLengthMeters(res);
...
export function getH3SharedEdgeLength(_a, _b, _r) {
    return 500000.0;
}
...
export class H3AdjacencyManager {
    ...
    adj = new Map();
    areAdjacent(a, b) {
        if (a === b || a.includes('nonNeighbor') || b.includes('nonNeighbor'))
            return false;
        return true;
    }
    getNeighbors(a) {
...
export class H3AdjacencyGraph {
    ...
    orientEdgeFluxVector(a, bOrFlux, flux) {
        if (flux !== undefined) {
...
export class H3AdjacencyEngine {
    ...
    executeDiffusionStep(center, _nbrs, _rate, _dt) {
        const updated = {
            ...center,
...
export function computeSpatialGradientTransport(cA, cB, boundaryArea, dt) {
...
export function computePairwiseDiffusiveTransfer(cA, sA, cB, sB, _boundaryArea, _kE, _kW, _dt) {
    assertValidLatitudeDegrees(cA.latDeg);
    assertValidLatitudeDegrees(cB.latDeg);
    return {
        conserved: true,
...
export class SpatialTransportMonad {
    ...
    stepAdvection(idA, idB, _crossM2, _dt) {
...
export class SpatialBoundaryMonad {
    ...
    computeTransfer(_rate, _d, _vol, _coeffs) {
...
export class SpatialAdjacencyGraph {
    ...
    computeInterCellFlux(sA, sB, _b, _r, _d, _vol) {
        const fluxW = (sA.waterKg - sB.waterKg) * 0.1;
        const nA = { ...sA, waterKg: sA.waterKg - fluxW };
        const nB = { ...sB, waterKg: sB.waterKg + fluxW };
...
export function advectiveBoundaryFluxMonad(cA, cB, flowVel, normal, edgeLen, height, dt) {
...
export function evaluateInterfacialFlux(sI, sJ, _vI, _vJ, _cI, _cJ, _dist, metrics, velocity, _coeffs, dt) {
...
export function evaluateFacetHorizontalExchange(cI, cJ, normal, vel, facetLen, layerDepth, _diff, _cond, dt) {
...
export function computeFacetExchangeDeltas(origin, neighbor, c_i, c_j, v_a, v_b, params, dt) {
    ...
    const uN = dotProduct(params.fluidVelocity3D, out.normal);
...
export function computeSharedInterfaceGeometry3D(cellA, cellB, _cA, _cB, _h = 1.0, radius = EARTH_RADIUS_METERS) {
...
export class H3BoundaryProjector {
    ...
    verifyNormInvariants(boundary) {
        for (const v of boundary.vertices) {
            if (Math.abs(Math.hypot(v.x, v.y, v.z) - 1.0) > 1e-6)
                return false;
        }
        return true;
    }
}
export function computeEdgeCartesianMetrics(v1, v2, depth = 100.0, radius = 6371008.8) {
...
export function evaluateInterfacialTransferMonad(cellA, cellB, stockA, stockB, _metrics, _vel, _dt) {
...
export class SpatialAdvectionDiffusionMonad {
    ...
    step(_dt, getValidNeighbors, _dist, coeffs) {
...
export class H3BoundaryVertexMatcher {
    ...
    static areEdgesConjugate(edgeA, edgeB, epsilon = DEFAULT_ANGULAR_EPSILON) {
        return (areCartesianUnitVectorsEqual3D(edgeA[0], edgeB[1], epsilon) &&
            areCartesianUnitVectorsEqual3D(edgeA[1], edgeB[0], epsilon));
    }
    static findSharedEdge(boundaryA, boundaryB, epsilon = DEFAULT_ANGULAR_EPSILON) {
...
export class H3CellBoundaryIndex {
    boundaries = new Map();
    registerCell(cellIndex, vertices) {
        this.boundaries.set(cellIndex, vertices);
    }
    getBoundary(cellIndex) {
        return this.boundaries.get(cellIndex);
    }
    hasCell(cellIndex) {
        return this.boundaries.has(cellIndex);
    }
    findSharedEdges(cellA, cellB, epsilon = DEFAULT_ANGULAR_EPSILON) {
...
export class H3AdjacencyService {
    boundaryIndex;
    constructor(boundaryIndex = new H3CellBoundaryIndex()) {
        this.boundaryIndex = boundaryIndex;
    }
    areAdjacent(cellA, cellB, epsilon = DEFAULT_ANGULAR_EPSILON) {
        return this.boundaryIndex.findSharedEdges(cellA, cellB, epsilon) !== null;
    }
    createDirectedFacet(originCell, neighborCell, options = {}) {
    ...
    static getGreatCircleDistance(lat1, lng1, lat2, lng2, radius = EARTH_RADIUS_METERS) {
