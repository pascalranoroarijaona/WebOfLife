...
import { assertPentagonalNeighborArrayType, assertPentagonDegree, validatePentagonAdjacency, H3AdjacencyGraph, PentagonalFluxMonad } from '../src/spatial/h3_adjacency.js';
describe('Sprint 080: assertPentagonalNeighborArrayType & Topology Defensive Guards', () => {
    ...
    describe('assertPentagonalNeighborArrayType - Non-array rejections', () => {
        it('should throw TypeError for null with expected diagnostic message', () => {
            assert.throws(() => assertPentagonalNeighborArrayType(null), (err) => {
        ...
        it('should throw TypeError for undefined', () => {
            assert.throws(() => assertPentagonalNeighborArrayType(undefined), (err) => {
                assert(err instanceof TypeError);
        ...
        it('should throw TypeError for string primitives', () => {
            assert.throws(() => assertPentagonalNeighborArrayType('8828308281fffff'), (err) => {
                assert(err instanceof TypeError);
        ...
        it('should throw TypeError for numeric values', () => {
            assert.throws(() => assertPentagonalNeighborArrayType(42), (err) => {
                assert(err instanceof TypeError);
        ...
        it('should throw TypeError for booleans', () => {
            assert.throws(() => assertPentagonalNeighborArrayType(true), (err) => {
                assert(err instanceof TypeError);
        ...
        it('should throw TypeError for plain objects and array-like structures', () => {
            assert.throws(() => assertPentagonalNeighborArrayType({ 0: 'a', 1: 'b', length: 2 }), (err) => {
                assert(err instanceof TypeError);
                assert(err.message.includes('Expected an Array, received object.'));
                return true;
            });
            assert.throws(() => assertPentagonalNeighborArrayType({}), (err) => {
                assert(err instanceof TypeError);
                assert(err.message.includes('Expected an Array, received object.'));
                return true;
            });
        ...
        it('should throw TypeError for Set and Map instances', () => {
            assert.throws(() => assertPentagonalNeighborArrayType(new Set(['a', 'b'])), (err) => {
                assert(err instanceof TypeError);
                assert(err.message.includes('Expected an Array, received object.'));
                return true;
            });
            assert.throws(() => assertPentagonalNeighborArrayType(new Map()), (err) => {
                assert(err instanceof TypeError);
                assert(err.message.includes('Expected an Array, received object.'));
                return true;
            });
        ...
        it('should throw TypeError for functions and symbols', () => {
            ...
            assert.throws(() => assertPentagonalNeighborArrayType(Symbol('pentagon')), (err) => {
                assert(err instanceof TypeError);
    ...
    describe('assertPentagonDegree & validatePentagonAdjacency', () => {
        ...
        it('should throw RangeError if neighbor count exceeds 5', () => {
            assert.throws(() => assertPentagonDegree([1, 2, 3, 4, 5, 6], 5), (err) => {
    ...
    describe('PentagonalFluxMonad Thermodynamic Invariants & Guard Execution', () => {
        ...
        const initialNeighbors = new Map([
