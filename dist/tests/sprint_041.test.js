...
import { extractUniqueCanonicalH3Tokens, isValidH3CellString, H3Grid, SpatialTelemetryIngestor } from '../src/spatial/h3_grid.js';
...
describe('Sprint 041: Canonical H3 Token Extraction Helper', () => {
    ...
    it('safely handles empty, null, undefined, and non-string inputs', () => {
        ...
        assert.deepStrictEqual(extractUniqueCanonicalH3Tokens(null), []);
        assert.deepStrictEqual(extractUniqueCanonicalH3Tokens(undefined), []);
        assert.deepStrictEqual(extractUniqueCanonicalH3Tokens(42), []);
    ...
    it('verifies thermodynamic mass conservation across spatial telemetry ingestion', () => {
        ...
        const result = SpatialTelemetryIngestor.ingestSafely(state, telemetryLog, (token, currentState) => {
