...
import { computeBoundarySegmentRadialNormal3D, computeBoundarySegmentRadialNormal3DFromPoints, computeBoundarySegmentTangent3D, computeBoundaryFacetFrame3D, createBoundarySegment3D, createVec3D, toVec3D, } from '../src/spatial/h3_adjacency.js';
const EPSILON_TOLERANCE = 1e-12;
function assertVectorClose(actual, expected, tolerance = EPSILON_TOLERANCE) {
    const act = toVec3D(actual);
    const exp = toVec3D(expected);
    for (let i = 0; i < 3; i++) {
        assert.ok(Math.abs(act[i] - exp[i]) <= tolerance, `Component [${i}] mismatch: actual=${act[i]}, expected=${exp[i]}, diff=${Math.abs(act[i] - exp[i])}`);
    }
}
...
describe('Sprint 062: RFC-062 Boundary Segment Radial Normal 3D Unit Vector', () => {
    it('computes radial normal correctly for equatorial segment ([1, 0, 0], [0, 1, 0])', () => {
        ...
        assertVectorClose(normal, createVec3D(expectedVal, expectedVal, 0));
    ...
    it('computes radial normal for Northern Arctic facet ([0, 1, 1], [1, 0, 1])', () => {
        const v1 = createVec3D(0, 1, 1);
        const v2 = createVec3D(1, 0, 1);
        const segment = createBoundarySegment3D(v1, v2);
        const normal = computeBoundarySegmentRadialNormal3D(segment);
        const sqrt6 = Math.sqrt(6);
        const expected = createVec3D(1 / sqrt6, 1 / sqrt6, 2 / sqrt6);
    ...
    it('computes radial normal for Polar Apex Parallel ([-0.5, 0, 1], [0.5, 0, 1])', () => {
        const v1 = createVec3D(-0.5, 0, 1);
        const v2 = createVec3D(0.5, 0, 1);
        const segment = createBoundarySegment3D(v1, v2);
        const normal = computeBoundarySegmentRadialNormal3D(segment);
        assertVectorClose(normal, createVec3D(0, 0, 1));
    });
    it('handles antipodal degeneracy safely without NaN or Infinity', () => {
        const v1 = createVec3D(1, 0, 0);
        const v2 = createVec3D(-1, 0, 0);
        const segment = createBoundarySegment3D(v1, v2);
        const normal = computeBoundarySegmentRadialNormal3D(segment);
        assert.deepStrictEqual(normal, createVec3D(0, 0, 1), 'Should return fallback polar zenith [0, 0, 1]');
    });
    it('handles zero-length vector degeneracy at origin safely', () => {
        const v1 = createVec3D(0, 0, 0);
        const v2 = createVec3D(0, 0, 0);
        const segment = createBoundarySegment3D(v1, v2);
        const normal = computeBoundarySegmentRadialNormal3D(segment);
        assert.deepStrictEqual(normal, createVec3D(0, 0, 1));
    });
    it('satisfies scale invariance: scaling vertices does not alter radial normal', () => {
        const v1 = createVec3D(1, 0, 0);
        const v2 = createVec3D(0, 1, 0);
        const v1Scaled = createVec3D(2, 0, 0);
        const v2Scaled = createVec3D(0, 2, 0);
    ...
    it('satisfies orthogonality to boundary tangent for spherical edges (||v1|| = ||v2||)', () => {
        ...
        const theta1 = 0.2;
        const phi1 = 0.5;
        const v1 = createVec3D(Math.sin(theta1) * Math.cos(phi1), Math.sin(theta1) * Math.sin(phi1), Math.cos(theta1));
        const theta2 = 0.4;
        const phi2 = 0.8;
        const v2 = createVec3D(Math.sin(theta2) * Math.cos(phi2), Math.sin(theta2) * Math.sin(phi2), Math.cos(theta2));
        const segment = createBoundarySegment3D(v1, v2);
        ...
        assert.ok(Math.abs(dot) < 1e-12, `Radial normal and tangent must be orthogonal, got dot product: ${dot}`);
    });
