...
import { computeBoundarySegmentRadialNormal3D, computeBoundarySegmentRadialNormal3DFromPoints, computeBoundarySegmentTangent3D, computeBoundaryFacetFrame3D, createBoundarySegment3D, } from '../src/spatial/h3_adjacency.js';
const EPSILON_TOLERANCE = 1e-12;
function assertVectorClose(actual, expected, tolerance = EPSILON_TOLERANCE) {
    for (let i = 0; i < 3; i++) {
        assert.ok(Math.abs(actual[i] - expected[i]) <= tolerance, `Component [${i}] mismatch: actual=${actual[i]}, expected=${expected[i]}, diff=${Math.abs(actual[i] - expected[i])}`);
    }
}
...
describe('Sprint 062: RFC-062 Boundary Segment Radial Normal 3D Unit Vector', () => {
    ...
    it('computes radial normal for Northern Arctic facet ([0, 1, 1], [1, 0, 1])', () => {
        ...
        const expected = [1 / sqrt6, 1 / sqrt6, 2 / sqrt6];
    ...
    it('satisfies scale invariance: scaling vertices does not alter radial normal', () => {
        const v1 = [1, 0, 0];
        const v2 = [0, 1, 0];
        const v1Scaled = [2, 0, 0];
        const v2Scaled = [0, 2, 0];
