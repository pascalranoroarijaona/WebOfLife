...
import { computeBoundaryCentroidDisplacement3D, computeDetailedCentroidDisplacement3D, executeAdvectiveBoundaryTransfer, H3AdjacencyManager, vectorNorm3D, } from "../src/spatial/h3_adjacency.js";
describe("Sprint 065 - computeBoundaryCentroidDisplacement3D", () => {
    ...
    it("TC-01: Equator Eastward 90 degrees displacement", () => {
        ...
        assert.ok(Math.abs(detailed.angularDistanceRad - Math.PI / 2.0) < TOLERANCE, `Angular distance should be pi/2, got ${detailed.angularDistanceRad}`);
    });
    it("TC-02: Equator to North Pole displacement", () => {
        const origin = { lat: 0.0, lng: 0.0 };
        const target = { lat: 90.0, lng: 0.0 };
        const u = computeBoundaryCentroidDisplacement3D(origin, target);
        const detailed = computeDetailedCentroidDisplacement3D(origin, target);
        const expectedVal = 1.0 / Math.sqrt(2.0);
    ...
    it("TC-03: Prime Meridian to Date Line (Antipodal)", () => {
        const origin = { lat: 0.0, lng: 0.0 };
        const target = { lat: 0.0, lng: 180.0 };
        const u = computeBoundaryCentroidDisplacement3D(origin, target);
        const detailed = computeDetailedCentroidDisplacement3D(origin, target);
    ...
    it("TC-04: Coincident Singular Centroid produces zero vector", () => {
        const coord = { lat: 37.77, lng: -122.41 };
    ...
    it("TC-05: Date-Line crossing boundary resolution", () => {
        const origin = { lat: 10.0, lng: 179.9 };
        const target = { lat: 10.0, lng: -179.9 };
        const u = computeBoundaryCentroidDisplacement3D(origin, target);
        const norm = vectorNorm3D(u);
    ...
    it("Unit Norm Invariant across 1,000 randomized spherical pairs", () => {
        for (let i = 0; i < 1000; i++) {
            ...
            const norm = vectorNorm3D(u);
            if (norm > 0) {
                assert.ok(Math.abs(norm - 1.0) < 1e-10, `Normalized vector must have norm 1.0, got ${norm}`);
            }
        }
    });
    ...
    it("Conservative Advective Boundary Transfer obeys zero-sum First Law", () => {
        ...
        assert.ok(Math.abs(finalTotalWater - initialTotalWater) < 1e-12, `Water mass must be strictly conserved. Initial: ${initialTotalWater}, Final: ${finalTotalWater}`);
        assert.ok(Math.abs(finalTotalEnergy - initialTotalEnergy) < 1e-12, `Thermal energy must be strictly conserved. Initial: ${initialTotalEnergy}, Final: ${finalTotalEnergy}`);
    });
});
