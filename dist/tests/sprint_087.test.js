...
import { PENTAGON_BASE_CELLS, PENTAGON_BASE_CELL_SET, determinePentagonBaseCellMissingDirection, isBaseCellPentagon, getBaseCellNeighbor, getPentagonDefectMetadata, verifyPentagonMissingDirectionConsistency, H3AdjacencyService, SpatialFluxMonad, DiscreteManifoldFluxMonad, TOTAL_BASE_CELLS } from '../src/spatial/h3_adjacency';
describe('Sprint 087: Pentagon Base Cell Missing Direction Mapping', () => {
    ...
    it('maps all 12 pentagon base cells to Direction.K_AXES', () => {
        for (const pentagon of EXPECTED_PENTAGONS) {
            ...
            assert.strictEqual(missingDir, Direction.K_AXES, `Pentagon ${pentagon} missing direction must be Direction.K_AXES (1)`);
        }
    });
    it('maps all 110 hexagon base cells to Direction.INVALID', () => {
        ...
        for (let bc = 0; bc < TOTAL_BASE_CELLS; bc++) {
            if (!PENTAGON_BASE_CELL_SET.has(bc)) {
                ...
                assert.strictEqual(missingDir, Direction.INVALID, `Hexagon ${bc} missing direction must be Direction.INVALID (7)`);
            }
        }
    ...
    it('returns Direction.INVALID for out-of-range, negative, and non-integer inputs', () => {
        ...
        for (const input of invalidInputs) {
            assert.strictEqual(determinePentagonBaseCellMissingDirection(input), Direction.INVALID, `Input ${input} should return Direction.INVALID`);
            assert.strictEqual(isBaseCellPentagon(input), false, `Input ${input} should not be identified as pentagon`);
        }
    });
    it('ensures getBaseCellNeighbor returns -1 along the omitted direction for pentagons', () => {
        for (const pentagon of EXPECTED_PENTAGONS) {
            ...
            assert.strictEqual(neighbor, -1, `Pentagon ${pentagon} neighbor in direction ${missingDir} must be -1`);
            assert.strictEqual(verifyPentagonMissingDirectionConsistency(pentagon), true, `Pentagon ${pentagon} should pass consistency verification`);
        }
    });
    ...
    it('conserves total mass in SpatialFluxMonad without leakage through omitted directions', () => {
        ...
        for (const pentagon of EXPECTED_PENTAGONS) {
            ...
            assert.ok(Math.abs(totalAllocated - testFlux) < 1e-12, `Mass allocated (${totalAllocated}) must equal input flux (${testFlux})`);
        }
    ...
    it('strictly satisfies First Law mass and energy conservation in DiscreteManifoldFluxMonad', () => {
        ...
        assert.ok(audit.omittedDirectionBoundaryCollisionsPrevented > 0, 'Must prevent boundary collisions across omitted directions on pentagon defects');
        assert.ok(Math.abs(audit.totalWaterDeltaKg) < 1e-9, `Water stock delta must be invariant: ${audit.totalWaterDeltaKg}`);
        assert.ok(Math.abs(audit.totalEnergyDeltaJoules) < 1e-6, `Energy stock delta must be invariant: ${audit.totalEnergyDeltaJoules}`);
        assert.ok(Math.abs(audit.totalCarbonDeltaKg) < 1e-9, `Carbon stock delta must be invariant: ${audit.totalCarbonDeltaKg}`);
    });
});
