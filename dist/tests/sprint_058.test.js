...
import { computeBoundaryMidpointLatLng, computeGreatCircleDistance, computeInitialBearing, computeMidpointCoriolis, computeMidpointSolarIrradiance, evaluateBoundaryInterface, SpatialBoundaryMonad, SpatialAdjacencyGraph, } from '../src/spatial/h3_adjacency.js';
describe('Sprint 058 - Spherical Boundary Midpoint & Interfacial Adjacency', () => {
    ...
    it('strictly preserves conservative mass and energy stocks across interface (First Law)', () => {
        const boundary = evaluateBoundaryInterface('origin', 'neighbor', { lat: 10, lng: 20 }, { lat: 11, lng: 21 });
        const state1 = {
            carbonKg: 5000,
            waterKg: 20000,
            oxygenKg: 10000,
        ...
        assert.ok(Math.abs((next1.carbonKg + next2.carbonKg) - (state1.carbonKg + state2.carbonKg)) < 1e-9, 'Carbon conservation violated');
        assert.ok(Math.abs((next1.waterKg + next2.waterKg) - (state1.waterKg + state2.waterKg)) < 1e-9, 'Water conservation violated');
        assert.ok(Math.abs((next1.oxygenKg + next2.oxygenKg) - (state1.oxygenKg + state2.oxygenKg)) < 1e-9, 'Oxygen conservation violated');
        assert.ok(Math.abs((next1.mineralsKg + next2.mineralsKg) - (state1.mineralsKg + state2.mineralsKg)) < 1e-9, 'Minerals conservation violated');
        assert.ok(Math.abs((next1.energyJoules + next2.energyJoules) - (state1.energyJoules + state2.energyJoules)) < 1e-4, 'Energy conservation violated');
    ...
    it('manages adjacency cache and executes inter-cell flux through SpatialAdjacencyGraph', () => {
        ...
        const hexA = 'hexA';
        const hexB = 'hexB';
        const boundary = evaluateBoundaryInterface(hexA, hexB, { lat: 40.0, lng: -74.0 }, { lat: 40.5, lng: -73.5 });
        ...
        const [nextA, nextB, deltas] = graph.computeInterCellFlux(stateA, stateB, retrievedBoundary, 2.0, 500, 300);
