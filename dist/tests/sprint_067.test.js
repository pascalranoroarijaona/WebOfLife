...
import { computeDetailedInterfaceNormal, computeInterfaceTransfer, EARTH_RADIUS_METERS, } from '../src/spatial/h3_adjacency.js';
describe('Sprint 067 - RFC-067 Interface Normal and Conservative Advection', () => {
    ...
    it('computes accurate interface normal between adjacent equatorial cells', () => {
        ...
        const centroidA = [r * Math.cos(rad(-1)), r * Math.sin(rad(-1)), 0];
        const centroidB = [r * Math.cos(rad(1)), r * Math.sin(rad(1)), 0];
        // Shared boundary vertices at lon = 0 deg, lat = -0.5 deg to +0.5 deg
        const vertexA = [r * Math.cos(rad(-0.5)), 0, r * Math.sin(rad(-0.5))];
        const vertexB = [r * Math.cos(rad(0.5)), 0, r * Math.sin(rad(0.5))];
    ...
    it('computes accurate interface normal between meridian-adjacent cells', () => {
        ...
        const centroidA = [r * Math.cos(rad(-1)), 0, r * Math.sin(rad(-1))];
        const centroidB = [r * Math.cos(rad(1)), 0, r * Math.sin(rad(1))];
        // Shared boundary vertices along latitude 0 from lon = -0.5 deg to +0.5 deg
        const vertexA = [r * Math.cos(rad(-0.5)), r * Math.sin(rad(-0.5)), 0];
        const vertexB = [r * Math.cos(rad(0.5)), r * Math.sin(rad(0.5)), 0];
        const result = computeDetailedInterfaceNormal(centroidA, centroidB, vertexA, vertexB, r);
    ...
    it('strictly conserves mass across all stocks during interfacial transport', () => {
        ...
        const velocity = [0, 5.0, 0]; // 5 m/s eastward
        const diffCoeff = 1.0;
        const thermalCond = 25.0;
        const heatCap = 1005.0;
        const dt = 60.0; // 60 seconds
        const transfer = computeInterfaceTransfer(metric, cellA, cellB, velocity, diffCoeff, thermalCond, heatCap, dt);
    ...
    it('strictly conserves thermal energy and satisfies the Second Law of Thermodynamics', () => {
        ...
        const transfer = computeInterfaceTransfer(metric, cellA, cellB, [0, 2.0, 0], 0.5, 50.0, 1000.0, 30.0);
        // Conservation of energy
        assert.ok(Math.abs(transfer.deltaOrigin.thermalEnergyJoules + transfer.deltaDestination.thermalEnergyJoules) < 1e-4, 'Thermal energy must be strictly conserved between origin and destination');
        // Second Law: Non-negative entropy production
        assert.ok(transfer.entropyGeneratedJPerK >= 0, `Entropy generated must be >= 0, got ${transfer.entropyGeneratedJPerK}`);
        assert.ok(transfer.entropyGeneratedJPerK > 0, 'Entropy generated must be strictly positive in presence of finite temperature gradient');
    });
    it('correctly adapts upwind donor concentration when velocity reverses', () => {
        ...
        const reversedVelocity = [0, -10.0, 0];
        const transfer = computeInterfaceTransfer(metric, cellA, cellB, reversedVelocity, 0.0, // pure advection
        0.0, 1000.0, 10.0);
