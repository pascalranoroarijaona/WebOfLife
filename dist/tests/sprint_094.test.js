...
import { countClassIIIApertureSteps, isClassIIIResolution, getApertureClassProfile, H3DirectionalKernel, SpatialFluxMonad, APERTURE_7_ROTATION_RAD, } from '../src/spatial/h3_adjacency.js';
describe('Sprint 094: Aperture-7 Class III Step Counter & Parity Engine', () => {
    describe('1. Base Case Verification (Resolution 0 up to Target Resolution)', () => {
        ...
        it('should match the full RFC analytical step table from 0 to 15', () => {
            ...
            for (let r = 0; r <= 15; r++) {
                assert.strictEqual(countClassIIIApertureSteps(r), expectedStepsFromZero[r], `Failed step count at resolution ${r}`);
            }
        });
    });
    ...
    describe('5. Thermodynamic and Directional Invariance', () => {
        it('H3DirectionalKernel preserves vector norm (energy conservation) under rotation', () => {
            ...
            assert.ok(Math.abs(initialNorm - rotatedNorm) < 1e-12, `Norm conservation violated: initial=${initialNorm}, rotated=${rotatedNorm}`);
        });
