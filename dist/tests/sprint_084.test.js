...
import { H3DirectionBitmask, DirectionalFluxOperator, } from '../src/spatial/h3_types.js';
describe('Sprint 084 - RFC-084: H3 Direction Bitmask Specification & Directional Adjacency', () => {
    it('verifies bitwise disjointness and completeness of all 6 directional constants', () => {
        ...
        const orSum = H3DirectionBitmask.DIRECTION_0 |
    ...
    it('tests hasDirection query functionality across single and combined bitmasks', () => {
        const mask0 = H3DirectionBitmask.DIRECTION_0;
        ...
        const multiMask = H3DirectionBitmask.DIRECTION_1 | H3DirectionBitmask.DIRECTION_4;
    ...
    it('tests setDirection and clearDirection idempotent mutations', () => {
        let mask = H3DirectionBitmask.NONE;
        // Set direction 2
        ...
        assert.strictEqual(mask, H3DirectionBitmask.DIRECTION_2 | H3DirectionBitmask.DIRECTION_5);
        // Clear direction 2
    ...
    it('tests reciprocal bitmask inversion with invertMask', () => {
        ...
        assert.strictEqual(inv1_4, H3DirectionBitmask.DIRECTION_4 | H3DirectionBitmask.DIRECTION_1);
        // Invert ALL gives ALL
        assert.strictEqual(H3DirectionBitmask.invertMask(H3DirectionBitmask.ALL), H3DirectionBitmask.ALL);
        // Invert NONE gives NONE
        assert.strictEqual(H3DirectionBitmask.invertMask(H3DirectionBitmask.NONE), H3DirectionBitmask.NONE);
        // Double inversion is identity for all 64 bitmask combinations
        for (let m = 0; m < 64; m++) {
            assert.strictEqual(H3DirectionBitmask.invertMask(H3DirectionBitmask.invertMask(m)), m);
        }
    });
    it('evaluates directional channel permeability gating with DirectionalFluxOperator', () => {
        ...
        assert.strictEqual(DirectionalFluxOperator.isChannelPermeable(srcMask, tgtMask, 1), true);
        // Source open, target blocked
        const tgtBlocked = H3DirectionBitmask.DIRECTION_0;
        assert.strictEqual(DirectionalFluxOperator.isChannelPermeable(srcMask, tgtBlocked, 1), false);
        // Source blocked, target open
        assert.strictEqual(DirectionalFluxOperator.isChannelPermeable(H3DirectionBitmask.NONE, tgtMask, 1), false);
        // Both fully open
        assert.strictEqual(DirectionalFluxOperator.isChannelPermeable(H3DirectionBitmask.ALL, H3DirectionBitmask.ALL, 0), true);
    });
    it('verifies conservative thermodynamic edge transfer when impermeable vs permeable', () => {
        ...
        const blockedTransfer = DirectionalFluxOperator.computeEdgeTransfer(stateI, stateJ, H3DirectionBitmask.NONE, H3DirectionBitmask.ALL, 2, // direction 2
        1.5, // 1.5 m/s velocity
        0.01, 0.6, geometry, 60);
        ...
        const permeableTransfer = DirectionalFluxOperator.computeEdgeTransfer(stateI, stateJ, srcOpen, tgtOpen, 2, 1.5, 0.01, 0.6, geometry, 60);
