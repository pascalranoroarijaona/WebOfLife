...
import { latLngToUnitVector3D, unitVectorDotProduct, unitVectorCrossProduct, unitVectorAngularDistance, unitVectorChordDistance, unitVectorTangentChord, H3AdjacencyMatrix } from '../src/spatial/h3_adjacency.js';
import { SpatialMonad, updatePlanetaryInsolation, SOLAR_CONSTANT_W_M2, MOLAR_MASS_C, MOLAR_MASS_CO2 } from '../src/monads/spatial_monad.js';
describe('Sprint 052: Spherical Unit Vector Projections (latLngToUnitVector3D)', () => {
    ...
    it('2. Norm Invariance across 10,000 randomized planetary coordinates', () => {
        ...
        for (let i = 0; i < SAMPLE_COUNT; i++) {
            ...
            assert.ok(diff <= 1.0e-14, `Norm drift exceeded tolerance at lat=${lat}, lng=${lng}: ||u|| - 1 = ${diff}`);
        }
    });
...
describe('Sprint 052: SpatialMonad & Thermodynamic Insolation Coupling', () => {
    it('1. First Law Energy Accounting and Stoichiometric Mass Balance', () => {
        const subsolarVector = [1.0, 0.0, 0.0]; // Subsolar point at Equator / PM
        ...
        assert.ok(Math.abs(actualEnergyInflux - expectedEnergyInflux) < 1.0e-4, `Thermal energy influx mismatch: got ${actualEnergyInflux}, expected ${expectedEnergyInflux}`);
        ...
        assert.ok(balanceDiff < 1.0e-9, `Stoichiometric balance violated: diff=${balanceDiff}`);
        // Transpiration water balance
        assert.ok(updatedCell.stocks.atmosphericWaterKg > initialCell.stocks.atmosphericWaterKg, 'Transpiration water must increase atmospheric water vapor stock');
        // Oxygen balance
        assert.ok(updatedCell.stocks.oxygenKg > initialCell.stocks.oxygenKg, 'Oxygen stock must increase proportionally to carbon fixation');
    });
    it('2. Hemispheric Solar Insolation Discretized Integral', () => {
        ...
        const subsolarVector = [0.0, 0.0, 1.0]; // Subsolar at North Pole
        const N_LAT = 180;
        const N_LNG = 360;
        ...
        assert.ok(error < 1.0e-3, `Discretized hemispheric integral diverged from 0.25: got ${averageCosZenith}, error=${error}`);
    });
});
