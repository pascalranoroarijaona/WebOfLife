...
describe('Sprint 024: Thermodynamic State Vector Interface Contracts & Laws', () => {
    ...
    it('should verify exact exergy destruction rate calculation (I = T_0 * S_gen)', () => {
        ...
        assert.strictEqual(Math.abs(result.exergyDestructionRate - expectedExergyDestruction) < 1e-6, true, `Exergy destruction rate I (${result.exergyDestructionRate}) must equal T_0 * S_gen (${expectedExergyDestruction})`);
    ...
    it('should throw or clamp on negative entropy generation scenarios to strictly enforce Second Law', () => {
        ...
        assert.ok(result.entropyGenerationRate >= 0, `Second Law invariant violated: entropyGenerationRate was ${result.entropyGenerationRate}`);
    });
});
