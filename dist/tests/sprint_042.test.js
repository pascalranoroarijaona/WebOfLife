...
import { H3CellThermodynamicRecord, H3StateTensorContainer, evaluateRadiativeStep, evaluatePhaseTransitions, stepThermodynamicCell, computeStefanBoltzmannLongwave, computeCompositeHeatCapacity, computeInternalEnergy } from '../src/spatial/h3_state_tensor.js';
...
describe('Sprint 042 - H3 Spatial Thermodynamic State Tensor', () => {
    ...
    const validRecord = new H3CellThermodynamicRecord(sampleCellId, areaM2, elevationM, initialEnergyJ, initialTempK, heatCapacityJK, 0.3, // albedo
    0.98, // emissivity
    ...
    it('rejects unphysical temperatures <= 0 K', () => {
        assert.throws(() => validRecord.withUpdates({ temperatureK: 0.0 }), /temperature must be strictly > 0 K/);
        assert.throws(() => validRecord.withUpdates({ temperatureK: -15.0 }), /temperature must be strictly > 0 K/);
    });
    it('rejects negative elemental mass stocks', () => {
        assert.throws(() => validRecord.withUpdates({ dryAirMassKg: -1.0 }), /mass stocks must be non-negative/);
        assert.throws(() => validRecord.withUpdates({ carbonMassKg: -0.5 }), /mass stocks must be non-negative/);
        assert.throws(() => validRecord.withUpdates({ liquidWaterMassKg: -10.0 }), /mass stocks must be non-negative/);
    });
    it('rejects water mass partition non-closure', () => {
        assert.throws(() => new H3CellThermodynamicRecord(sampleCellId, areaM2, elevationM, initialEnergyJ, initialTempK, heatCapacityJK, 0.3, 0.98, 342.0, 102.6, 380.0, 15.0, 80.0, 1.2e8, 0.05, 1.0e7, 3.0e5, // Mismatched total: 3.0e5 != 2.0e5 + 0.0 + 5.0e4 (2.5e5)
        2.0e5, 0.0, 5.0e4, 1.0e4, 5.0e3, 1.0e2), /water mass closure failure/);
    });
    it('rejects albedo and emissivity outside valid physical interval [0, 1]', () => {
        assert.throws(() => validRecord.withUpdates({ albedo: 1.05 }), /albedo must be in \[0.0, 1.0\]/);
        assert.throws(() => validRecord.withUpdates({ albedo: -0.01 }), /albedo must be in \[0.0, 1.0\]/);
        assert.throws(() => validRecord.withUpdates({ emissivity: 1.2 }), /emissivity must be in \[0.0, 1.0\]/);
        assert.throws(() => validRecord.withUpdates({ emissivity: -0.1 }), /emissivity must be in \[0.0, 1.0\]/);
    });
    it('verifies First Law mass summation and flux computations', () => {
        const expectedMass = validRecord.dryAirMassKg +
    ...
    it('operates H3StateTensorContainer and aggregates conserved quantities', () => {
        ...
        assert.strictEqual(container.computeTotalInternalEnergyJ(), validRecord.internalEnergyJ + cell2.internalEnergyJ);
        assert.strictEqual(container.computeTotalMassKg(), validRecord.totalMassKg + cell2.totalMassKg);
        assert.strictEqual(container.computeTotalEntropyJPerK(), validRecord.entropyJPerK + cell2.entropyJPerK);
    });
    ...
    it('evaluates water phase transition freezing and melting equilibrium', () => {
        ...
        assert.ok(Math.abs(frozen.totalWaterMassKg - (frozen.liquidWaterMassKg + frozen.iceMassKg + frozen.vaporMassKg)) < 1e-6);
    ...
    it('executes stepThermodynamicCell with inter-cell boundary advection', () => {
        ...
        assert.strictEqual(stepped.dryAirMassKg, validRecord.dryAirMassKg + boundary.dryAirFluxInKgPerS * dt);
    ...
    it('verifies helper functions: heat capacity and internal energy derivations', () => {
        const cComposite = computeCompositeHeatCapacity(1.0e7, 2.0e5, 0.0, 5.0e4, areaM2);
