...
import { APERTURE_7_ROTATION_RAD, countClassIIIApertureSteps, computeClassIIIRotationAngleRadians, H3SpatialTransformationRegistry, Aperture7GridCoordinateTransformer, } from "../src/spatial/h3_adjacency.js";
...
describe("Sprint 095: Hierarchical Aperture-7 Class III Coordinate Rotation Angle Computation", () => {
    ...
    describe("Aperture Constant & Fundamental Properties", () => {
        it("should export the correct numerical constant for APERTURE_7_ROTATION_RAD", () => {
            ...
            assert.ok(Math.abs(APERTURE_7_ROTATION_RAD - theoretical) < EPSILON, `Expected ${theoretical}, received ${APERTURE_7_ROTATION_RAD}`);
    ...
    describe("countClassIIIApertureSteps", () => {
        it("returns 0 for equal start and target resolutions", () => {
            for (let r = 0; r <= 15; r++) {
                assert.strictEqual(countClassIIIApertureSteps(r, r), 0, `Resolution ${r} -> ${r} must have 0 steps`);
            }
        });
        ...
        it("satisfies global antisymmetry: count(r1, r2) === -count(r2, r1)", () => {
            for (let r1 = 0; r1 <= 15; r1++) {
                for (let r2 = 0; r2 <= 15; r2++) {
                    assert.strictEqual(countClassIIIApertureSteps(r1, r2), -countClassIIIApertureSteps(r2, r1), `Antisymmetry failed for (${r1}, ${r2})`);
                }
            }
        });
        ...
        it("exhibits antisymmetry under reversal", () => {
            for (let r1 = 0; r1 <= 15; r1 += 3) {
                for (let r2 = 0; r2 <= 15; r2 += 3) {
                    ...
                    assert.ok(Math.abs(fwd + rev) < 1e-10, `Antisymmetry failed for ${r1} <-> ${r2}: ${fwd} vs ${rev}`);
                }
            }
        });
    ...
    describe("Aperture7GridCoordinateTransformer & Registry", () => {
        it("instantiates and computes rotation angles correctly", () => {
            ...
            assert.ok(Math.abs(transformer.getRotationAngle() - 2 * APERTURE_7_ROTATION_RAD) < EPSILON);
            assert.ok(Math.abs(transformer.computeRotationAngle() - 2 * APERTURE_7_ROTATION_RAD) < EPSILON);
        });
        it("rotates vector preserving Euclidean magnitude (First Law of Thermodynamics)", () => {
            ...
            assert.ok(Math.abs(normBefore - normAfter) < EPSILON, `Norm must be conserved. Before: ${normBefore}, After: ${normAfter}`);
        });
    ...
    describe("SpatialFluxMonad Inter-Resolution Integration", () => {
        it("aligns flux across resolutions preserving magnitude and stocks", () => {
            ...
            const initialVector = { x: 30.0, y: 40.0 };
            ...
            assert.ok(Math.abs(initialNorm - alignedNorm) < EPSILON, `Vector norm must remain conserved across resolution alignment`);
        });
        it("conserves mass and internal energy across cell boundary stock transfers", () => {
            ...
            const boundaryNormal = { x: 1.0, y: 0.0 };
            const boundaryLength = 100; // meters
            const dt = 10; // seconds
            const [updatedSource, updatedTarget] = monad.transferStocksAcrossBoundary(targetStocks, boundaryNormal, boundaryLength, dt);
            // Mass conservation check: total before === total after
            assert.ok(Math.abs(sourceStocks.carbonKg + targetStocks.carbonKg -
                (updatedSource.carbonKg + updatedTarget.carbonKg)) < EPSILON);
            assert.ok(Math.abs(sourceStocks.waterKg + targetStocks.waterKg -
                (updatedSource.waterKg + updatedTarget.waterKg)) < EPSILON);
            assert.ok(Math.abs(sourceStocks.internalEnergyJoules + targetStocks.internalEnergyJoules -
                (updatedSource.internalEnergyJoules + updatedTarget.internalEnergyJoules)) < EPSILON);
        });
    });
    describe("SO(2) Orthogonality and Invariant Audit", () => {
        it("ensures det(R) === 1 and R^T * R === I", () => {
            for (let r = 0; r <= 15; r++) {
                ...
                assert.ok(Math.abs(det - 1.0) < EPSILON, `det(R) must equal 1.0 for angle ${theta}`);
