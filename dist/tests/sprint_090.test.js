...
import { isPurePentagonResolutionIndex, isPentagonBaseCell, isPentagon, computePentagonBoundaryDelta, getPentagonNeighborDirections, APERTURE_ROTATION_RAD, APERTURE_ROTATION_DEG } from '../src/spatial/h3_adjacency';
import { PENTAGON_BASE_CELLS } from '../src/spatial/h3_types';
import { buildH3Index } from '../src/spatial/h3_grid';
describe('Sprint 090 - Pure Pentagon Resolution Verification', () => {
    describe('1. Numeric Resolution Overload Invariants', () => {
        it('returns true for all Class II even resolutions (0, 2, 4, 6, 8, 10, 12, 14)', () => {
            ...
            for (const res of evenResolutions) {
                assert.strictEqual(isPurePentagonResolutionIndex(res), true, `Resolution ${res} should be recognized as a pure pentagon resolution`);
            }
        });
        it('returns false for all Class III odd resolutions (1, 3, 5, 7, 9, 11, 13, 15)', () => {
            ...
            for (const res of oddResolutions) {
                assert.strictEqual(isPurePentagonResolutionIndex(res), false, `Resolution ${res} has aperture rotation and must not be pure`);
            }
        });
    ...
    describe('5. Thermodynamic First Law Conservation & Boundary Delta Formulation', () => {
        ...
        it('strictly satisfies First Law mass and enthalpy conservation across boundary', () => {
            ...
            const deltas = computePentagonBoundaryDelta(purePentagon, neighbor, sourceStocks, neighborStocks, 1000, // 1000 meters face length
        ...
        it('bypasses aperture rotation for pure pentagons and applies rotation for Class III pentagons', () => {
            ...
            const neighbor = buildH3Index(2, 4, [1, 0]);
            const pureDeltas = computePentagonBoundaryDelta(purePentagon, neighbor, sourceStocks, neighborStocks, 1000, 2000, 10.0, 0.0, // purely advective to isolate Vn
            1);
            const oddDeltas = computePentagonBoundaryDelta(oddPentagon, neighbor, sourceStocks, neighborStocks, 1000, 2000, 10.0, 0.0, 1);
            ...
            assert.ok(Math.abs(actualRatio - expectedRatio) < 1e-10, `Expected ratio ${expectedRatio}, got ${actualRatio}`);
        });
    });
});
