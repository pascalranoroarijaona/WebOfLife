...
import { computeBoundaryOutwardNormal3D, computeFacetExchangeDeltas, vec3Dot, vec3Norm, vec3Normalize, vec3Add, vec3Sub, latLngToCartesian3D, cartesian3DToLatLng, H3AdjacencyGraph, WGS84_EARTH_MEAN_RADIUS_METERS } from '../src/spatial/h3_adjacency.js';
describe('Sprint 066 - computeBoundaryOutwardNormal3D & Lateral Facet Transport', () => {
    const EPSILON = 1e-12;
    // Helper to create unit sphere vectors
    function makeUnit(x, y, z) {
    ...
    describe('4. Anti-Symmetry & Flux Conservation Invariant', () => {
        it('satisfies normal_ij + normal_ji = 0 when order of cells and boundary vertices are inverted', () => {
            ...
            assert.ok(Math.abs(forward.midpoint.x - backward.midpoint.x) < EPSILON &&
    ...
    describe('8. Lateral Facet Stock Transfer Monad (Thermodynamics & First Law)', () => {
        ...
        it('satisfies exact zero-sum mass and energy conservation across facet (First Law)', () => {
            ...
            const res = computeFacetExchangeDeltas(originState, neighborState, c_i, c_j, v_a, v_b, params, dt);
        ...
        it('preserves non-negative entropy generation (Second Law)', () => {
            ...
            assert.ok(res.originDeltas.entropyProductionJoulesPerKelvin >= 0, `Expected non-negative entropy production, got ${res.originDeltas.entropyProductionJoulesPerKelvin}`);
            assert.ok(res.neighborDeltas.entropyProductionJoulesPerKelvin >= 0, `Expected non-negative entropy production, got ${res.neighborDeltas.entropyProductionJoulesPerKelvin}`);
        });
    });
