...
import { extractH3BoundaryCartesianVertices3D, SpatialGeometryBridge, H3BoundaryProjector, H3AdjacencyGraph, computeEdgeCartesianMetrics, evaluateInterfacialTransferMonad } from '../src/spatial/h3_adjacency.js';
import { SpatialFluxMonad } from '../src/spatial/spatial_flux_monad.js';
// Helper to obtain resilient H3 hexagon test index
function getTestHexagon(res = 7) {
    if (typeof h3.latLngToCell === 'function') {
        return h3.latLngToCell(37.7749, -122.4194, res);
    }
    if (typeof h3.geoToH3 === 'function') {
        return h3.geoToH3(37.7749, -122.4194, res);
    }
    return '872830828ffffff';
}
// Helper to obtain resilient H3 pentagon test index
function getTestPentagon(res = 7) {
    if (typeof h3.getPentagons === 'function') {
        const p = h3.getPentagons(res);
        if (p && p.length > 0)
            return p[0];
    }
    if (typeof h3.getPentagonIndexes === 'function') {
        const p = h3.getPentagonIndexes(res);
        if (p && p.length > 0)
            return p[0];
    }
...
describe('Sprint 069: extractH3BoundaryCartesianVertices3D & 3D Interfacial Topology', () => {
    it('extracts unclosed 3D Cartesian boundary vertices for an H3 hexagon', () => {
        ...
        for (const v of boundary.vertices) {
            ...
            assert.ok(Math.abs(norm - 1.0) < 1e-12, `Vertex norm ${norm} violates unit sphere invariant`);
        }
        // Centroid norm invariant
        const cNorm = Math.sqrt(boundary.centroid.x * boundary.centroid.x +
    ...
    it('correctly applies custom planetary scaling radius (Earth radius)', () => {
        ...
        for (const v of boundary.vertices) {
            const norm = Math.sqrt(v.x * v.x + v.y * v.y + v.z * v.z);
            assert.ok(Math.abs(norm - R_EARTH) < 1e-4, `Vertex norm ${norm} must match scaled radius ${R_EARTH}`);
        }
        const cNorm = Math.sqrt(boundary.centroid.x * boundary.centroid.x +
            boundary.centroid.y * boundary.centroid.y +
            boundary.centroid.z * boundary.centroid.z);
    ...
    it('validates centroid collinearity with cell center', () => {
        ...
        const centerCartesian = SpatialGeometryBridge.latLngToCartesian(centerLatLng[0], centerLatLng[1], 1.0);
        ...
        assert.ok(angularDiff < 0.05, `Centroid angular divergence ${angularDiff} rad exceeds 0.05 rad threshold`);
    });
    ...
    it('satisfies First Law mass conservation and Second Law entropy generation in SpatialFluxMonad', () => {
        ...
        const delta = evaluateInterfacialTransferMonad(cellA, cellB, stockA, stockB, metrics, velocityVec, 60.0);
