...
import { isPentagonCell, isExpectedNeighborCountForCell, getExpectedNeighborCount, assertValidNeighborCountForCell, validateAdjacencyInvariant, createCellAdjacencyState, calculateConservativeFluxStep, } from '../src/spatial/h3_adjacency.js';
describe('Sprint 077: Cell Neighbor Count Assertion Specification', () => {
    ...
    describe('assertValidNeighborCountForCell: VAL-TOPO-01 Type Safety', () => {
        it('throws TypeError if cellId is not a non-empty string', () => {
            assert.throws(() => assertValidNeighborCountForCell('', ['n1', 'n2', 'n3', 'n4', 'n5', 'n6']), (err) => err instanceof TypeError && /cellId/.test(err.message));
            assert.throws(() => assertValidNeighborCountForCell(null, []), (err) => err instanceof TypeError);
        });
        it('throws TypeError when neighbors is nullish, primitive, or non-array', () => {
            ...
            for (const nonArray of nonArrays) {
                assert.throws(() => assertValidNeighborCountForCell(mockHexagonId, nonArray), (err) => {
                    assert.ok(err instanceof TypeError);
                    assert.ok(err.message.includes('Expected neighbors to be an array'));
                    assert.ok(err.message.includes(mockHexagonId));
                    return true;
                });
            }
        });
    ...
    describe('assertValidNeighborCountForCell: VAL-TOPO-02 & VAL-TOPO-03 Valence Cardinality', () => {
        ...
        it('throws RangeError when a hexagon has invalid neighbor count (5, 7, 0)', () => {
            ...
            for (const count of invalidCounts) {
                ...
                assert.throws(() => assertValidNeighborCountForCell(mockHexagonId, neighbors), (err) => {
                    assert.ok(err instanceof RangeError);
                    assert.ok(err.message.includes(`Invalid neighbor count ${count}`));
                    assert.ok(err.message.includes(mockHexagonId));
                    assert.ok(err.message.includes('expected 6 for hexagon'));
                    return true;
                });
            }
        });
        it('throws RangeError when a pentagon has invalid neighbor count (4, 6, 0)', () => {
            ...
            for (const count of invalidCounts) {
                const neighbors = Array.from({ length: count }, (_, i) => `n${i}`);
                assert.throws(() => assertValidNeighborCountForCell(mockPentagonId, neighbors), (err) => {
                    assert.ok(err instanceof RangeError);
                    assert.ok(err.message.includes(`Invalid neighbor count ${count}`));
                    assert.ok(err.message.includes(mockPentagonId));
                    assert.ok(err.message.includes('expected 5 for pentagon'));
                    return true;
                });
            }
        });
    ...
    describe('validateAdjacencyInvariant & createCellAdjacencyState', () => {
        ...
        it('throws TypeError if neighbor collection contains non-strings', () => {
            const mixedNeighbors = ['c1', 42, 'c3', 'c4', 'c5', 'c6'];
            assert.throws(() => validateAdjacencyInvariant(mockHexagonId, mixedNeighbors), (err) => err instanceof TypeError && /non-string|number/.test(err.message));
        });
    });
    describe('VAL-TOPO-04 Conservative Flux Step Integration', () => {
        it('calculates anti-symmetric conservative transfers over validated adjacency', () => {
            ...
            const targetStates = hexNeighbors.map((id, idx) => createCellAdjacencyState(id, Array.from({ length: 6 }, (_, i) => (i === 0 ? mockHexagonId : `sub_n_${idx}_${i}`))));
            const params = {
                transmissivity: 1.5,
                conductivity: 2.0,
