...
import { calculateH3BoundaryContactArea, getH3SharedEdgeLength, H3AdjacencyManager, } from '../src/spatial/h3_adjacency.js';
import { computeLateralBoundaryTransfer, } from '../src/monads/spatial_monad.js';
import { EARTH_AUTHALIC_RADIUS_METERS, } from '../src/thermodynamics/constants.js';
...
describe('Sprint 050 - calculateH3BoundaryContactArea & Lateral Flux Dynamics', () => {
    ...
    it('1. Geometric Adjacency & Boundary Length: should detect adjacency and return positive edge length', () => {
        ...
        assert.ok(ratio > 0.6 && ratio < 1.5, `Calculated edge length (${edgeLength}) should be close to nominal (${nominal})`);
    });
    it('2. Symmetry Invariant: A(u, v) === A(v, u) to machine precision', () => {
        const stratumA = { zBaseMeters: -100, zTopMeters: 500 };
        const stratumB = { zBaseMeters: 100, zTopMeters: 1000 };
        ...
        assert.strictEqual(resAB.contactAreaM2, resBA.contactAreaM2, 'Contact area must be strictly symmetric: Area(A, B) === Area(B, A)');
        assert.strictEqual(resAB.boundaryLengthMeters, resBA.boundaryLengthMeters, 'Boundary length must be strictly symmetric');
    });
    it('3. Non-Adjacent & Self-Intersection: should yield zero contact area', () => {
        const stratum = { zBaseMeters: 0, zTopMeters: 100 };
        // Self cell
        ...
        const nonNeighborRes = calculateH3BoundaryContactArea(cellA, stratum, nonNeighborCell, stratum);
    ...
    it('4. Disjoint and Inverted Strata: should yield zero overlap and zero area', () => {
        ...
        const stratumA = { zBaseMeters: 0, zTopMeters: 100 };
        const stratumB = { zBaseMeters: 200, zTopMeters: 300 };
        ...
        const invertedStratumA = { zBaseMeters: 100, zTopMeters: 0 };
        const invertedRes = calculateH3BoundaryContactArea(cellA, invertedStratumA, cellB, stratumA);
    ...
    it('5. Spherical Radial Expansion Scaling: gamma = 1 + z_mid / R', () => {
        const stratumLow = { zBaseMeters: 0, zTopMeters: 1000 };
        const stratumHigh = { zBaseMeters: 50_000, zTopMeters: 51_000 };
        const optionsExpanded = { applyRadialExpansion: true };
        const optionsUnexpanded = { applyRadialExpansion: false };
        const resUnexpanded = calculateH3BoundaryContactArea(cellA, stratumHigh, cellB, stratumHigh, optionsUnexpanded);
        const resExpanded = calculateH3BoundaryContactArea(cellA, stratumHigh, cellB, stratumHigh, optionsExpanded);
        ...
        assert.ok(Math.abs(actualRatio - expectedGamma) < 1e-6, `Expansion ratio ${actualRatio} should match theoretical gamma ${expectedGamma}`);
    });
    it('6. Class-based Architecture: H3BoundaryContactCalculator & H3AdjacencyManager', () => {
        ...
        const stratum = { zBaseMeters: 0, zTopMeters: 200 };
        ...
        const overlap = calc.calculateVerticalOverlap({ zBaseMeters: 10, zTopMeters: 50 }, { zBaseMeters: 30, zTopMeters: 80 });
    ...
    it('7. Thermodynamic Invariance: Conservation of Mass and Energy in Lateral Transport', () => {
        const stratumA = { zBaseMeters: 0, zTopMeters: 100 };
        const stratumB = { zBaseMeters: 0, zTopMeters: 100 };
        const initialStocksA = {
        ...
        const transfer = computeLateralBoundaryTransfer(cellA, stratumA, initialStocksA, cellB, stratumB, initialStocksB, params);
        ...
        assert.ok(Math.abs(deltaSumWater) < 1e-9, `Water conservation violated: sum = ${deltaSumWater}`);
        assert.ok(Math.abs(deltaSumCarbon) < 1e-9, `Carbon conservation violated: sum = ${deltaSumCarbon}`);
        assert.ok(Math.abs(deltaSumOxygen) < 1e-9, `Oxygen conservation violated: sum = ${deltaSumOxygen}`);
        assert.ok(Math.abs(deltaSumMinerals) < 1e-9, `Mineral conservation violated: sum = ${deltaSumMinerals}`);
        assert.ok(Math.abs(deltaSumEnergy) < 1e-7, `Energy conservation violated: sum = ${deltaSumEnergy}`);
