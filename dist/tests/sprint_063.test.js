...
import { createVec3D, computeBoundaryHorizontalNormal3D, computeBoundaryHorizontalNormalFromEndpoints3D, computeBoundaryDarbouxFrame3D, computeSharedBoundaryMidpoint3D, evaluateFacetHorizontalExchange, dotProduct3D, vectorNorm3D, } from '../src/spatial/h3_adjacency.js';
const EPSILON = 1e-12;
describe('Sprint 063: computeBoundaryHorizontalNormal3D & Facet Transport', () => {
    it('computes correct horizontal normal for equatorial boundary edge', () => {
        ...
        const tangent = createVec3D(0, 1, 0);
        const radial = createVec3D(1, 0, 0);
    ...
    it('computes correct horizontal normal for meridional boundary edge', () => {
        ...
        const tangent = createVec3D(0, 0, 1);
        const radial = createVec3D(1, 0, 0);
        const normal = computeBoundaryHorizontalNormal3D(tangent, radial);
        assert.ok(Math.abs(normal.x - 0) < EPSILON);
    ...
    it('satisfies unit norm invariance across arbitrary spherical orientations', () => {
        ...
        for (const { theta, phi } of orientations) {
            // Radial unit normal
            const radial = createVec3D(Math.sin(theta) * Math.cos(phi), Math.sin(theta) * Math.sin(phi), Math.cos(theta));
            // Local tangent east vector (-sin(phi), cos(phi), 0)
            const tangent = createVec3D(-Math.sin(phi), Math.cos(phi), 0);
            const normal = computeBoundaryHorizontalNormal3D(tangent, radial);
            const norm = vectorNorm3D(normal);
            assert.ok(Math.abs(norm - 1.0) < EPSILON, `Expected unit norm, got ${norm} at theta=${theta}, phi=${phi}`);
        }
    });
    it('satisfies strict orthogonality with tangent and radial normal', () => {
        const tangent = createVec3D(0.5773502691896258, -0.5773502691896258, 0.5773502691896258);
        ...
        const radial = createVec3D(rawR.x / rNorm, rawR.y / rNorm, 0);
        const normal = computeBoundaryHorizontalNormal3D(tangent, radial);
        const dotTangent = dotProduct3D(normal, tangent);
        const dotRadial = dotProduct3D(normal, radial);
        assert.ok(Math.abs(dotTangent) < EPSILON, `Expected dot(n_h, t) == 0, got ${dotTangent}`);
        assert.ok(Math.abs(dotRadial) < EPSILON, `Expected dot(n_h, r) == 0 (zero vertical leakage), got ${dotRadial}`);
    });
    it('safely handles degenerate inputs (parallel vectors or zero vectors)', () => {
        const vZero = createVec3D(0, 0, 0);
    ...
    it('evaluates boundary horizontal normal from endpoints and midpoint directly', () => {
        ...
        const v1 = createVec3D(R * Math.cos(lon1), R * Math.sin(lon1), 0);
        const v2 = createVec3D(R * Math.cos(lon2), R * Math.sin(lon2), 0);
    ...
    it('constructs a full orthonormal Darboux frame', () => {
        const R = 6.371e6;
        const v1 = createVec3D(R, 0, 0);
        const v2 = createVec3D(0, R, 0);
    ...
    it('governs conservative facet exchange and non-negative entropy production', () => {
        ...
        const velocity = createVec3D(0, 5.0, 0); // 5 m/s toward cell J
        const facetLength = 100; // m
        const layerDepth = 50; // m
        const dt = 10; // s
        const diffusivity = 1e-4;
        const thermalConductivity = 0.6;
        const exchangeItoJ = evaluateFacetHorizontalExchange(cellI, cellJ, normal, velocity, facetLength, layerDepth, diffusivity, thermalConductivity, dt);
        ...
        assert.ok(exchangeItoJ.entropyProduction >= 0, `Entropy production should be non-negative, got ${exchangeItoJ.entropyProduction}`);
    });
});
