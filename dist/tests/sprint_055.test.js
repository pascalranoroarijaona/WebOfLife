...
import { normalizeAngleRadians, HexagonalAdvectiveBearing, computeAdvectiveEdgeTransfer, computeGeodesicBearing, } from "../src/spatial/h3_adjacency.js";
...
describe("Sprint 055: Algebraic & Range Invariants", () => {
    ...
    it("satisfies idempotence: ψ(ψ(θ)) = ψ(θ)", () => {
        ...
        for (const val of testValues) {
            ...
            assert.ok(Math.abs(once - twice) < 1e-15, `Idempotence failed for ${val}: once=${once}, twice=${twice}`);
        }
    });
    it("satisfies 2π periodicity across multiple integer multiples", () => {
        ...
        for (let k = -10; k <= 10; k++) {
            ...
            assert.ok(Math.abs(wrapped - expected) < 1e-14, `Periodicity failed for k=${k}: got ${wrapped}, expected ${expected}`);
        }
    });
});
...
describe("Sprint 055: Advective Stock Flux & Spatial Monad Conservation", () => {
    ...
    it("monadic advection strictly conserves First Law mass and energy", () => {
        ...
        assert.ok(Math.abs(initialTotalCarbon - finalTotalCarbon) < 1e-9, "Carbon must be conserved");
        assert.ok(Math.abs(initialTotalEnergy - finalTotalEnergy) < 1e-4, "Energy must be conserved");
    });
