...
import {
  ...
  computeSpatialGradientTransport,
  H3AdjacencyMatrix,
...
describe('Sprint 046 - Geodesic Haversine Distance Metric', () => {
  ...
  it('computes New York to Tokyo geodesic baseline within 0.2%', () => {
    ...
    assert.ok(
      Math.abs(distanceKm - 10850) < 10850 * 0.01,
      `Expected ~10,850 km, got ${distanceKm}`
    );
  });
  ...
  it('verifies metric symmetry: d(A, B) === d(B, A)', () => {
    const pointA: [number, number] = [37.7749, -122.4194]; // San Francisco
    const pointB: [number, number] = [-33.8688, 151.2093]; // Sydney
  ...
  it('verifies the triangle inequality: d(A, C) <= d(A, B) + d(B, C)', () => {
    const pA: [number, number] = [10.0, 20.0];
    const pB: [number, number] = [25.0, 55.0];
    const pC: [number, number] = [-15.0, 80.0];
...
describe('Sprint 046 - H3AdjacencyMatrix Geodesic Integration', () => {
  ...
  it('throws informative error when distance is queried for unknown centroids', () => {
    ...
    assert.throws(
      () => matrix.getCentroidDistance('unknown_1', 'unknown_2'),
...
describe('Sprint 046 - Thermodynamic Gradient Transport Monad', () => {
  it('satisfies First Law conservation of energy, water, and carbon stocks', () => {
    const cellA: CellThermodynamicState = {
      cellIndex: 'cell_A',
    ...
    const cellB: CellThermodynamicState = {
      cellIndex: 'cell_B',
    ...
    assert.strictEqual(
      delta.deltaInternalEnergyJoulesA + delta.deltaInternalEnergyJoulesB,
      0.0,
      'Energy is strictly conserved across cell interface'
    );

    // 2. First Law: Mass Conservation (Water Vapor)
    assert.strictEqual(
      delta.deltaWaterVaporKgA + delta.deltaWaterVaporKgB,
      0.0,
      'Water mass is strictly conserved'
    );

    // 3. First Law: Mass Conservation (Carbon)
    assert.strictEqual(
      delta.deltaCarbonKgA + delta.deltaCarbonKgB,
      0.0,
      'Carbon mass is strictly conserved'
    );

    // 4. Second Law: Non-negative Entropy Generation
    assert.ok(
      delta.entropyGeneratedJoulesPerKelvin >= 0.0,
