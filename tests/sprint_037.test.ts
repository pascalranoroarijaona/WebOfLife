...
import {
  H3_CANONICAL_INDEX_PATTERN,
  isValidH3CanonicalIndex,
  assertCanonicalH3Index,
  verifyH3PatternContract,
  H3CellCoord,
  H3GridManager,
...
describe('Sprint 037: H3_CANONICAL_INDEX_PATTERN & Spatial Validation Suite', () => {
  ...
  it('assertCanonicalH3Index throws RangeError for malformed index', () => {
    assert.throws(
      () => assertCanonicalH3Index('invalid-hex-val'),
      (err: unknown) => err instanceof RangeError && /Invalid H3 canonical index/.test(err.message)
    );
  });
  ...
  it('SpatialCellMonad rejects non-physical stock initialization (NaN or negative)', () => {
    const invalidStocks: CellThermodynamicStocks = {
      waterKg: -10,
      carbonKg: 50,
      mineralKg: 20,
      oxygenKg: 10,
    ...
    assert.throws(
      () => SpatialCellMonad.unit('8826856235fffff', invalidStocks),
      /Thermodynamic invariant violation/
    );
  });

  it('executeAdvectiveTransfer obeys mass conservation invariant (Delta M_total = 0)', () => {
    const initialStocksSource: CellThermodynamicStocks = {
      waterKg: 1000,
      carbonKg: 500,
      mineralKg: 250,
      oxygenKg: 150,
    ...
    const initialStocksTarget: CellThermodynamicStocks = {
      waterKg: 200,
      carbonKg: 100,
      mineralKg: 50,
      oxygenKg: 30,
    ...
    const transferRequest: StockTransferDelta = {
      deltaWaterKg: 150,
      deltaCarbonKg: 50,
      deltaMineralKg: 25,
      deltaOxygenKg: 10,
  ...
  it('executeAdvectiveTransfer rejects self-advection transfers', () => {
    const stocks: CellThermodynamicStocks = {
      waterKg: 500,
      carbonKg: 200,
      mineralKg: 100,
      oxygenKg: 50,
    ...
    assert.throws(
      () => executeAdvectiveTransfer(cellA, cellB, transferRequest),
      /Self-advection transfer rejected/
    );
  });
});