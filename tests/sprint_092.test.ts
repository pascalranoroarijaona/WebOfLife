...
import {
  ...
  computeHexagonalMetrics,
  getNeighborsAtResolution,
  computeAdjacencyWeights,
  H3AdjacencyGraph,
  H3AdjacencyManager,
  SpatialFluxMonad,
  simulateConservativeFlux,
  BASE_CELL_AREA_M2,
  CellStocks,
  AdjacencyEdge,
...
describe('RFC-092: assertValidApertureResolution Boundary Enforcement', () => {
  ...
  it('TV-03: accepts standard intermediate resolutions [1, 7, 8, 14]', () => {
    const validLevels = [1, 7, 8, 14] as const;
  ...
  it('TV-04: throws InvalidApertureResolutionError for negative resolution -1', () => {
    assert.throws(
      () => assertValidApertureResolution(-1),
      (err: unknown) => {
        assert.ok(err instanceof InvalidApertureResolutionError);
        assert.ok(err instanceof RangeError);
        assert.strictEqual((err as InvalidApertureResolutionError).resolution, -1);
        assert.ok((err as InvalidApertureResolutionError).message.includes('Resolution cannot be negative'));
        return true;
      }
    );
  });
  ...
  it('TV-05: throws InvalidApertureResolutionError for resolution exceeding 15 (16)', () => {
    assert.throws(
      () => assertValidApertureResolution(16),
      (err: unknown) => {
        assert.ok(err instanceof InvalidApertureResolutionError);
        assert.strictEqual((err as InvalidApertureResolutionError).resolution, 16);
        assert.ok((err as InvalidApertureResolutionError).message.includes('Resolution exceeds maximum H3 aperture'));
        return true;
      }
    );
  });
  ...
  it('TV-06: throws InvalidApertureResolutionError for fractional float (3.14159)', () => {
    assert.throws(
      () => assertValidApertureResolution(3.14159),
      (err: unknown) => {
        assert.ok(err instanceof InvalidApertureResolutionError);
        assert.strictEqual((err as InvalidApertureResolutionError).resolution, 3.14159);
        assert.ok((err as InvalidApertureResolutionError).message.includes('Value must be an integer'));
        return true;
      }
    );
  });
  ...
  it('TV-07: throws InvalidApertureResolutionError for NaN', () => {
    assert.throws(
      () => assertValidApertureResolution(NaN),
      (err: unknown) => {
        assert.ok(err instanceof InvalidApertureResolutionError);
        assert.ok((err as InvalidApertureResolutionError).message.includes('Value must be a finite number'));
        return true;
      }
    );
  });
  ...
  it('TV-08: throws InvalidApertureResolutionError for +Infinity and -Infinity', () => {
    assert.throws(
      () => assertValidApertureResolution(Infinity),
      (err: unknown) => {
        assert.ok(err instanceof InvalidApertureResolutionError);
        assert.ok((err as InvalidApertureResolutionError).message.includes('Value must be a finite number'));
        return true;
  ...
  it('Verifies First Law (Mass & Energy Conservation) and Second Law (Entropy Production >= 0)', () => {
    ...
    const cells = new Map<string, CellStocks>([
      ['cell_hot', cellA],
      ['cell_cold', cellB],
    ]);
    ...
    const initialTotalMass =
      cellA.carbonKg + cellA.waterKg + cellA.oxygenKg + cellA.mineralsKg +
    ...
    const finalTotalMass =
      finalA.carbonKg + finalA.waterKg + finalA.oxygenKg + finalA.mineralsKg +
    ...
    assert.ok(
      result.deltas.entropyProductionJoulesPerKelvin >= 0,
  ...
  it('SpatialFluxMonad preserves resolution and enforces assertion in pipeline', () => {
    ...
    const flatMapped = doubled.flatMap((st) =>
      SpatialFluxMonad.bindAtResolution({ biomassKg: st.biomassKg + 500 }, 8)
    );
