...
describe('Sprint 076: Topological Adjacency Validation & Discrete Flux Conservation', () => {
  ...
  const validHexId = typeof (h3 as any).latLngToCell === 'function'
    ? (h3 as any).latLngToCell(37.7749, -122.4194, 1)
    : (typeof (h3 as any).geoToH3 === 'function'
      ? (h3 as any).geoToH3(37.7749, -122.4194, 1)
      : '81283ffffffffff');
  ...
  it('AC-076-06: Non-array or null/undefined neighbor argument evaluates to false', () => {
    assert.strictEqual(isExpectedNeighborCountForCell(validHexId, null as unknown as string[]), false);
    assert.strictEqual(isExpectedNeighborCountForCell(validHexId, undefined as unknown as string[]), false);
    assert.strictEqual(isExpectedNeighborCountForCell(validHexId, {} as unknown as string[]), false);
    assert.strictEqual(isExpectedNeighborCountForCell(validHexId, 'string' as unknown as string[]), false);
    assert.strictEqual(isExpectedNeighborCountForCell(validHexId, 6 as unknown as string[]), false);
  });
  ...
  it('AC-076-07: Functional composition delegates to isExpectedNeighborCount', () => {
    ...
    assert.strictEqual(
      isExpectedNeighborCountForCell(validHexId, arr6),
      isExpectedNeighborCount(validHexId, 6)
    );
    assert.strictEqual(
      isExpectedNeighborCountForCell(validHexId, arr5),
      isExpectedNeighborCount(validHexId, 5)
    );
    assert.strictEqual(
      isExpectedNeighborCountForCell(validHexId, arr0),
      isExpectedNeighborCount(validHexId, 0)
    );
    assert.strictEqual(
      isExpectedNeighborCountForCell(validPentagonId, arr5),
      isExpectedNeighborCount(validPentagonId, 5)
    );
    assert.strictEqual(
      isExpectedNeighborCountForCell(validPentagonId, arr6),
      isExpectedNeighborCount(validPentagonId, 6)
    );
  });
  ...
  it('TopologicalFluxMonad integration: evaluates divergence when neighbor topology is complete', () => {
    const centerStock: CellStockState = {
      carbonKg: 100,
      waterKg: 200,
      mineralsKg: 50,
      oxygenKg: 80,
    ...
    const neighborStock: CellStockState = {
      carbonKg: 120,
      waterKg: 190,
      mineralsKg: 55,
      oxygenKg: 85,
    ...
    const conductance: BoundaryConductance = {
    ...
    const mapIncomplete = new Map<string, CellStockState>();
    for (const n of incompleteNeighbors) mapIncomplete.set(n, neighborStock);

    const failResult = monad.evaluateDivergence(
      incompleteNeighbors,
      mapIncomplete,
      conductance,
      diffusivity,
    ...
    const mapComplete = new Map<string, CellStockState>();
    for (const n of completeNeighbors) mapComplete.set(n, neighborStock);

    const successResult = monad.evaluateDivergence(
      completeNeighbors,
      mapComplete,
      conductance,
      diffusivity,
