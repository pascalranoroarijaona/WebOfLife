...
import { SpatialMonad, CellNodeData } from "../src/monads/spatial_monad.js";
...
describe("RFC-060: Tangent Space Projection Core Differential Geometry", () => {
  ...
  it("Identity 2: Pure Radial Cancellation - radial vector is entirely eliminated", () => {
    ...
    assert.ok(
      normResult < 1e-12,
      `Expected result norm to be near zero, got ${normResult}`
    );
  });
  ...
  it("Identity 3: Orthogonal Decomposition - ||v_perp||^2 + ||v_par||^2 == ||v||^2", () => {
    ...
    assert.ok(
      diff < 1e-9,
      `Pythagorean decomposition failed: ||v||^2=${normV2}, sum=${normPerp2 + normPar2}, diff=${diff}`
    );
    ...
    assert.ok(
      relativeOrthogonality < 1e-13,
      `Projected vector not orthogonal to origin position: relative error ${relativeOrthogonality}`
    );
  });
  ...
  it("Identity 4: Polar and Equatorial Consistency", () => {
    ...
    assert.ok(
      Math.abs(resultPole[2]) < 1e-12,
      `Z-velocity at North pole should be stripped to 0, got ${resultPole[2]}`
    );
    ...
    assert.ok(
      Math.abs(resultEquator[0]) < 1e-12,
      `X-velocity at Equator should be stripped to 0, got ${resultEquator[0]}`
    );
...
describe("RFC-060: H3 Facet Tangent Basis & Adjacency Engine", () => {
  it("computes facet normal basis between adjacent cells on sphere", () => {
    ...
    assert.ok(
      Math.abs(normalMagnitude - 1.0) < 1e-9,
      `Tangent normal magnitude must be 1, got ${normalMagnitude}`
    );
  ...
  it("H3AdjacencyGraphEngine manages topology and local projections", () => {
    ...
    const c1: Vector3D = latLngToCartesian(10, 20);
    const c2: Vector3D = latLngToCartesian(10, 21);
...
describe("RFC-060: Conservative Spherical Advection & Stock Invariance", () => {
  it("computeInterfaceAdvectiveTransfer conserves mass between two cells with radial-corrupted velocities", () => {
    ...
    const cellB: CellAdvectionState = {
      h3Index: "B",
      centroid: cB,
      area: 1e8,
      velocity: vB_raw,
    ...
    assert.ok(
      Math.abs(normalVelocity) < 20,
      `Normal velocity should be physical (~5 m/s), not contaminated by 10000 m/s radial. Got ${normalVelocity}`
    );
  ...
  it("SpatialMonad preserves exact mass and energy across multi-step spherical advection cycle", () => {
    ...
    const initialStocks1: CellStocks = {
      carbon: 1000,
      water: 5000,
      nitrogen: 300,
      phosphorus: 80,
    ...
    const initialStocks2: CellStocks = {
      carbon: 2000,
      water: 10000,
      nitrogen: 600,
      phosphorus: 160,
    ...
    const initialStocks3: CellStocks = {
      carbon: 3000,
      water: 15000,
      nitrogen: 900,
      phosphorus: 240,
