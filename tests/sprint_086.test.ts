...
import {
  extractPentagonApertureDigits,
  H3PentagonApertureParser,
  buildH3Index,
  SpatialFluxMonad,
} from "../src/spatial/h3_adjacency.js";
import { CellSpatialContext } from "../src/spatial/h3_types.js";
...
describe("Sprint 086 — Directional Aperture Parsing & Pentagonal Boundary Flux", () => {
  ...
  it("TC-08: Thermodynamic Invariant Conservation Across Pentagonal Singularity", () => {
    ...
    const source: CellSpatialContext = {
      h3Index: pentagonHex,
      state: {
        carbonKg: 500.0,
        waterKg: 1000.0,
    ...
    const neighbor: CellSpatialContext = {
      h3Index: neighborHex,
      state: {
        carbonKg: 300.0,
        waterKg: 800.0,
    ...
    const { updatedSource, updatedNeighbor, exchange } = SpatialFluxMonad.applyExchange(
      source,
      neighbor,
      2,
      60.0 // dt = 60s
    ...
    assert.ok(
      exchange.entropyGeneratedJPerK >= 0.0,
      `Second Law violated: sigma = ${exchange.entropyGeneratedJPerK}`
    );
