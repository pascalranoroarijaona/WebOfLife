...
import {
  ...
  calculateH3EdgeLengthAnalytical,
  createH3BoundaryInterface,
  getH3EdgeMetrics,
  computeBoundaryDiffusionStep,
  computeBoundaryThermalExchangeStep,
  computeBoundaryHydraulicExchangeStep,
  H3AdjacencyGraph,
  H3_NOMINAL_EDGE_LENGTH_TABLE
} from '../src/spatial/h3_adjacency.js';

import { H3Grid } from '../src/spatial/h3_grid.js';
import { SpatialMonad, ISpatialThermodynamicState } from '../src/monads/spatial_monad.js';
...
describe('Sprint 047: Spherical Geodesic Edge Scaling Architecture', () => {
  describe('calculateH3EdgeLengthMeters: Precision & Reference Invariants', () => {
    it('returns exact reference edge lengths for resolutions 0, 7, and 15 within ±0.01 tolerance', () => {
      ...
      assert.ok(
        Math.abs(edge0 - 1_107_712.59) < 0.01,
        `Expected res 0 to be ~1107712.59, got ${edge0}`
      );

      const edge7 = calculateH3EdgeLengthMeters(7);
      assert.ok(
        Math.abs(edge7 - 1_220.63) < 0.01,
        `Expected res 7 to be ~1220.63, got ${edge7}`
      );

      const edge15 = calculateH3EdgeLengthMeters(15);
      assert.ok(
        Math.abs(edge15 - 0.51) < 0.01,
        `Expected res 15 to be ~0.51, got ${edge15}`
      );
    });
    ...
    it('is strictly monotonic decreasing: L(r+1) < L(r) for all r in [0, 14]', () => {
      for (let r = 0; r < 15; r++) {
        ...
        assert.ok(
          ratio >= 2.4 && ratio <= 2.9,
          `Scaling ratio between resolution ${r} and ${r + 1} (${ratio}) deviated excessively from sqrt(7)`
        );
      }
    });
  ...
  describe('calculateH3EdgeLengthMeters: Validation & Error Handling', () => {
    ...
    it('throws RangeError for NaN, Infinity, and non-numeric inputs', () => {
      ...
      assert.throws(() => calculateH3EdgeLengthMeters(-Infinity), RangeError);
      assert.throws(() => calculateH3EdgeLengthMeters('7' as unknown as number), RangeError);
      assert.throws(() => calculateH3EdgeLengthMeters(null as unknown as number), RangeError);
    });
  });
  ...
  describe('Thermodynamic Mass-Energy Conservation Verification', () => {
    ...
    it('Process 3 (Hydraulic Conveyance): preserves total water mass and volume', () => {
      ...
      const result = computeBoundaryHydraulicExchangeStep(
        headSource,
        headTarget,
        waterDepthSource,
        waterDepthTarget,
      ...
      assert.ok(
        Math.abs(result.deltaVolumeM3Source + result.deltaVolumeM3Target) < 1e-12,
        'Volume conservation violated'
      );
      assert.ok(
        Math.abs(result.deltaMassKgSource + result.deltaMassKgTarget) < 1e-9,
  ...
  describe('H3Grid & SpatialMonad Integration', () => {
    it('initializes H3Grid with consistent aperture-7 metrics', () => {
      const grid = new H3Grid<{ vegetationIndex: number }>(7);
    ...
    it('performs monadic diffusion step conserving total solute across adjacent cells', () => {
      const stateA: ISpatialThermodynamicState = {
        massKg: 10_000,
        temperatureK: 295,
      ...
      const stateB: ISpatialThermodynamicState = {
        massKg: 10_000,
        temperatureK: 295,
