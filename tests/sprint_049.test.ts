...
describe('Sprint 049: Topological Pentagon Cell Validation', () => {
  it('Base Cell Resolution 0 Verification: Exactly 12 pentagons across all 122 base cells', () => {
    let pentagonCount = 0;
    const detectedPentagons: number[] = [];
    ...
    assert.deepStrictEqual(
      detectedPentagons,
      expectedPentagons,
      'Detected pentagon base cells must match canonical icosahedral set'
    );
  });
  ...
  it('Multi-Resolution Invariance: Center digit branch preserves pentagon status', () => {
    ...
    for (let res = 1; res <= 5; res++) {
      ...
      assert.strictEqual(
        isPentagonCell(pentagonIndex),
        true,
        `Base cell 4 at res ${res} with all 0 digits must be a pentagon`
      );
      assert.strictEqual(
        validator.getCoordinationNumber(pentagonIndex),
        5,
        `Coordination number must be 5 for pentagon at res ${res}`
      );
    }
  });
  ...
  it('Non-Zero Child Branch: Any non-zero digit transitions into regular hexagon', () => {
    ...
    for (let digit = 1; digit <= 6; digit++) {
      ...
      assert.strictEqual(
        isPentagonCell(hexIndex),
        false,
        `Child cell with digit ${digit} must not be a pentagon`
      );
      assert.strictEqual(
        getCoordinationNumber(hexIndex),
        6,
        `Hexagonal child cell must have coordination number 6`
      );
    }
  ...
  it('Invalid Index Resilience: Handles malformed strings, mode != 1, res > 15 gracefully', () => {
    ...
    assert.throws(
      () => validator.validateIndex(invalidModeIndex),
      /Invalid H3 mode/
    );
  });
  ...
  it('Boundary Flux & Second-Law Metric Scaling: Applies perimeter factor to pentagon interfaces', () => {
    ...
    assert.ok(
      Math.abs(pentToHexFlux.effectiveAreaM2 - expectedScaledArea) < 1e-6,
      'Effective area must scale by pentagon perimeter factor'
    );
    assert.ok(
      pentToHexFlux.massFlux > hexToHexFlux.massFlux,
      'Mass flux must scale proportionally to effective interface conductance'
    );
  });
  ...
  it('Mass and Energy Conservation: 100-step diffusive transport preserves machine precision', () => {
    ...
    const initialStates: CellStockState[] = [
      {
        h3Index: pentagonId,
        waterKg: 5000.0,
        carbonKg: 200.0,
    ...
    assert.ok(
      waterDeviation < 1e-11,
      `Water mass must be strictly conserved (deviation: ${waterDeviation} kg)`
    );
    assert.ok(
      carbonDeviation < 1e-12,
      `Carbon mass must be strictly conserved (deviation: ${carbonDeviation} kg)`
    );
    assert.ok(
      energyDeviation < 1e-6,
      `Thermal energy must be conserved (deviation: ${energyDeviation} J)`
    );
  });
});