...
import {
  ...
  computeCellHeatCapacity,
  computeCellChemicalEnthalpy,
...
describe('Sprint 045: Thermodynamic Overrides & Boundary Ledgering', () => {
  ...
  function generateH3Indices(count: number): string[] {
  ...
  it('2. Thermodynamic Limit Violations: strict mode rejects negative mass and sub-CMB temperature without mutating buffer', () => {
    ...
    assert.throws(
      () => {
        applyThermodynamicOverrides(
          tensor,
          { [cellIndices[2]]: { vegetationBiomassKg: -1.0 } },
    ...
    assert.throws(
      () => {
        applyThermodynamicOverrides(
          tensor,
          { [cellIndices[3]]: { temperatureKelvin: -5.0 } },
          { strictThermodynamicBounds: true }
        );
      },
      ThermodynamicDomainViolationError
    ...
    assert.throws(
      () => {
        applyThermodynamicOverrides(
          tensor,
          { '8828308281ffffff': { waterMassKg: 100 } },
          { strictThermodynamicBounds: true }
        );
      },
      CellOutOfBoundsError
  ...
  it('3. Clamping Mode (strict: false) clamps out-of-bounds inputs', () => {
    ...
    const report = applyThermodynamicOverrides(
      tensor,
      {
        [cellIndices[0]]: {
    ...
    assert.strictEqual(
      tensor.getCellValue(cellIndices[0], ThermodynamicChannel.TEMPERATURE_KELVIN),
      THERMODYNAMIC_CONSTANTS.MIN_TEMPERATURE_KELVIN
    );
  ...
  it('4. Thermal Coupling & Consistency: reconciling T and sensible heat', () => {
    ...
    assert.strictEqual(
      tensor.getCellValue(targetCell, ThermodynamicChannel.SENSIBLE_HEAT_JOULES),
      expectedHeatReconciled
    );
  });
  ...
  it('5. Chemical Enthalpy Ledgering when includeChemicalEnthalpy is true', () => {
    ...
    const expectedChemicalDelta =
      10.0 * THERMODYNAMIC_CONSTANTS.SPECIFIC_ENTHALPY.WATER +
  ...
  it('7. High-Volume Stability: 10,000 in-place partial mutations perform cleanly without buffer churn', () => {
    ...
    const batchMap = new Map<string, CellThermodynamicOverride>();
    for (let i = 0; i < count; i++) {
