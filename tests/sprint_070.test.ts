...
import {
  ...
  computeAngularDistance3D,
  normalizeVector3D,
  DEFAULT_ANGULAR_EPSILON,
  H3BoundaryVertexMatcher,
  H3CellBoundaryIndex,
  H3AdjacencyService,
} from '../src/spatial/h3_adjacency.js';
import {
  SpatialFluxMonad,
  CellBiogeochemicalStock,
...
describe('Sprint 070: areCartesianUnitVectorsEqual3D & Spatial Adjacency Verification', () => {
  ...
  it('9. H3BoundaryVertexMatcher deduplicates vertices and detects shared conjugate edges', () => {
    ...
    assert.strictEqual(
      areCartesianUnitVectorsEqual3D(shared.edgeA[0], shared.edgeB[1]),
      true
    );
    assert.strictEqual(
      areCartesianUnitVectorsEqual3D(shared.edgeA[1], shared.edgeB[0]),
      true
    );
  });
  ...
  it('11. SpatialFluxMonad enforces exact mass & energy conservation and positive entropy production', () => {
    const originStock: CellBiogeochemicalStock = {
      cellIndex: 'cell_A',
      carbonMol: 1000,
      nitrogenMol: 200,
      phosphorusMol: 50,
    ...
    const neighborStock: CellBiogeochemicalStock = {
      cellIndex: 'cell_B',
      carbonMol: 800,
      nitrogenMol: 150,
      phosphorusMol: 30,
    ...
    const facet: DirectedBoundaryFacet = {
      originCell: 'cell_A',
      neighborCell: 'cell_B',
    ...
    const result = SpatialFluxMonad.computeFacetTransfer(
      originStock,
      neighborStock,
      facet,
      60.0 // 60 seconds
    ...
    assert.ok(
      Math.abs(result.originDelta.deltaCarbonMol + result.neighborDelta.deltaCarbonMol) < 1e-12,
      'Carbon must be conserved'
    );
    assert.ok(
      Math.abs(result.originDelta.deltaNitrogenMol + result.neighborDelta.deltaNitrogenMol) < 1e-12,
      'Nitrogen must be conserved'
    );
    assert.ok(
      Math.abs(result.originDelta.deltaPhosphorusMol + result.neighborDelta.deltaPhosphorusMol) < 1e-12,
      'Phosphorus must be conserved'
    );
    assert.ok(
      Math.abs(result.originDelta.deltaWaterMol + result.neighborDelta.deltaWaterMol) < 1e-12,
      'Water must be conserved'
    );
    assert.ok(
      Math.abs(result.originDelta.deltaOxygenMol + result.neighborDelta.deltaOxygenMol) < 1e-12,
      'Oxygen must be conserved'
    );
    assert.ok(
      Math.abs(result.originDelta.deltaThermalEnergyJoules + result.neighborDelta.deltaThermalEnergyJoules) < 1e-6,
      'Thermal energy must be conserved'
    );

    // Second Law of Thermodynamics: Entropy production >= 0
    assert.ok(
      result.entropyProductionJPerK >= 0,
      `Entropy production (${result.entropyProductionJPerK}) must be non-negative`
    );
  });
  ...
  it('12. SpatialFluxMonad rejects non-conjugate boundary facets safely without leaking flux', () => {
    ...
    const nonConjugateFacet: DirectedBoundaryFacet = {
      originCell: 'cell_A',
      neighborCell: 'cell_B',
      originV1: { x: 1, y: 0, z: 0 },
      originV2: { x: 0, y: 1, z: 0 },
  ...
  it('13. Spherical to Cartesian conversions preserve angular equality', () => {
    ...
    assert.strictEqual(
      areCartesianUnitVectorsEqual3D(cart, reprojected, DEFAULT_ANGULAR_EPSILON),
      true
    );
  });
});