...
import {
  ...
  orderSharedBoundaryEndpointsByCentroid3D,
  H3AdjacencyGraph,
  createVec3D,
...
import { Point2D, Vector3D } from '../src/spatial/h3_types.js';
...
describe('Sprint 072 - RFC-072: Centroid-Relative Boundary Ordering & Outward Normal Orientation', () => {
  it('INV-072-1: Outward normal dot product with centroid displacement is strictly positive', () => {
    ...
    const centroidA: Point2D = [0, 0];
    const centroidB: Point2D = [2, 0];
    const p1: Point2D = [1, -1];
  ...
  it('INV-072-3: Unit length normal verification: ||n|| = 1.0', () => {
    const centroidA: Point2D = [3.5, 4.2];
    const centroidB: Point2D = [7.1, 8.9];
    // Arbitrary inclined edge
    const p1: Point2D = [5.0, 7.0];
    const p2: Point2D = [6.0, 5.5];
  ...
  it('3D Spherical S^2 ordering: correctly orients boundary and outward normal', () => {
    ...
    const centroidA: Vector3D = createVec3D(1, 0, 0);
    const centroidB: Vector3D = createVec3D(0, 1, 0);
    ...
    const p1: Vector3D = createVec3D(cos30 * Math.cos(rad45), cos30 * Math.sin(rad45), -sin30);
    const p2: Vector3D = createVec3D(cos30 * Math.cos(rad45), cos30 * Math.sin(rad45), sin30);
  ...
  it('Thermodynamic First Law: Exact skew-symmetric flux balance between two cells', () => {
    const stateA: BoundaryFluxState = {
    ...
    const stateB: BoundaryFluxState = {
    ...
    const centroidA: Point2D = [0, 0];
    const centroidB: Point2D = [10, 0];
    const p1: Point2D = [5, -5];
    const p2: Point2D = [5, 5];

    const fluxAtoB = computeOrientedEdgeFlux(
      stateA,
      stateB,
      centroidA,
      centroidB,
    ...
    const fluxBtoA = computeOrientedEdgeFlux(
      stateB,
      stateA,
      centroidB,
      centroidA,
  ...
  it('SpatialFluxMonad: Zero-sum conservation over multiple simulation steps', () => {
    ...
    const initialC1: BoundaryFluxState = {
    ...
    const initialC2: BoundaryFluxState = {
      thermalEnergyJoules: 100000,
      waterMassKg: 500,
      carbonMassKg: 50,
      oxygenMassKg: 25,
