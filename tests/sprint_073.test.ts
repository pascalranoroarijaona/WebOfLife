...
import { StateVector, ElementalKey } from '../src/thermodynamics/state_vector.js';
...
describe('Sprint 073 - Thermodynamic State Vector Discrepancy Absolute Difference Math Function', () => {
  it('should compute zero deltas for exact state matches', () => {
    const actual: StateVector = new StateVector({ stocks: { carbon: 850, nitrogen: 3900000, phosphorus: 4e9, water: 1338000000 } });
    const expected: StateVector = new StateVector({ stocks: { carbon: 850, nitrogen: 3900000, phosphorus: 4e9, water: 1338000000 } });
  ...
  it('should correctly handle positive and negative stock deviations resulting in absolute values', () => {
    const actual: StateVector = new StateVector({ stocks: { carbon: 900, nitrogen: 3800000, phosphorus: 4.1e9, water: 1300000000 } });
    const expected: StateVector = new StateVector({ stocks: { carbon: 850, nitrogen: 3900000, phosphorus: 4.0e9, water: 1338000000 } });
  ...
  it('should handle partial or missing elemental keys under strict type safety (defaulting to 0)', () => {
    const actual: StateVector = new StateVector({ stocks: { carbon: 500 } });
    const expected: StateVector = new StateVector({ stocks: { nitrogen: 200, carbon: 450 } });
