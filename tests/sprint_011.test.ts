...
describe('Sprint 011: Thermodynamic State Vector Interface & Second Law Compliance', () => {
  ...
  it('should integrate correctly with EarthPOD state vector reporting', () => {
    ...
    assert.ok(vector.entropyGenerationRate >= 0);
    assert.strictEqual(vector.exergyDestructionRate, (vector.referenceTemperature ?? STANDARD_AMBIENT_TEMPERATURE_K) * vector.entropyGenerationRate);
