...
import { Vector3D, Vector3DInput } from '../src/spatial/h3_types.js';

const EPSILON_TOLERANCE = 1e-12;

function assertVectorClose(actual: Vector3DInput, expected: Vector3DInput, tolerance: number = EPSILON_TOLERANCE) {
...
function vectorLength(v: Vector3DInput): number {
  const arr = toVec3D(v);
  return Math.sqrt(arr[0] * arr[0] + arr[1] * arr[1] + arr[2] * arr[2]);
}

function dotProduct(a: Vector3DInput, b: Vector3DInput): number {
  const va = toVec3D(a);
  const vb = toVec3D(b);
...
describe('Sprint 062: RFC-062 Boundary Segment Radial Normal 3D Unit Vector', () => {
  ...
  it('computes radial normal correctly for equatorial segment ([1, 0, 0], [0, 1, 0])', () => {
    ...
    const v2: Vector3D = createVec3D(0, 1, 0);
  ...
  it('computes radial normal for Northern Arctic facet ([0, 1, 1], [1, 0, 1])', () => {
    const v1: Vector3D = createVec3D(0, 1, 1);
    const v2: Vector3D = createVec3D(1, 0, 1);
    const segment = createBoundarySegment3D(v1, v2);

    const normal = computeBoundarySegmentRadialNormal3D(segment);
    const sqrt6 = Math.sqrt(6);
    const expected: Vector3D = createVec3D(1 / sqrt6, 1 / sqrt6, 2 / sqrt6);
  ...
  it('computes radial normal for Polar Apex Parallel ([-0.5, 0, 1], [0.5, 0, 1])', () => {
    const v1: Vector3D = createVec3D(-0.5, 0, 1);
    const v2: Vector3D = createVec3D(0.5, 0, 1);
    const segment = createBoundarySegment3D(v1, v2);
  ...
  it('handles antipodal degeneracy safely without NaN or Infinity', () => {
    const v1: Vector3D = createVec3D(1, 0, 0);
    const v2: Vector3D = createVec3D(-1, 0, 0);
    const segment = createBoundarySegment3D(v1, v2);
  ...
  it('handles zero-length vector degeneracy at origin safely', () => {
    const v1: Vector3D = createVec3D(0, 0, 0);
    const v2: Vector3D = createVec3D(0, 0, 0);
    const segment = createBoundarySegment3D(v1, v2);
  ...
  it('satisfies scale invariance: scaling vertices does not alter radial normal', () => {
    ...
    const v1Scaled: Vector3D = createVec3D(2, 0, 0);
    const v2Scaled: Vector3D = createVec3D(0, 2, 0);
