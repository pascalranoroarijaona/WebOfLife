...
import {
  ...
  computeInterfaceFlux,
  CellThermodynamicState,
  FluxComputationParams,
...
describe('Sprint 051: H3CellInterfaceMetrics Specification & Validation', () => {
  ...
  const sampleMetrics: H3CellInterfaceMetrics = createH3CellInterfaceMetrics({
  ...
  it('INV-METRIC-FLUX: verifies First Law mass and enthalpy conservation across reciprocal pairs', () => {
    ...
    const params: FluxComputationParams = {
      kSatPorous: 1e-4, // m/s
      manningN: 0.035,
    ...
    assert.ok(
      Math.abs(fluxAToB.deltaWaterKg + fluxBToA.deltaWaterKg) < 1e-9,
      `Water conservation error: ${fluxAToB.deltaWaterKg + fluxBToA.deltaWaterKg}`
    );
    assert.ok(
      Math.abs(fluxAToB.deltaEnthalpyJoules + fluxBToA.deltaEnthalpyJoules) < 1e-9,
      `Enthalpy conservation error: ${fluxAToB.deltaEnthalpyJoules + fluxBToA.deltaEnthalpyJoules}`
    );
    assert.ok(
      Math.abs(fluxAToB.deltaCarbonKg + fluxBToA.deltaCarbonKg) < 1e-9,
      `Carbon conservation error: ${fluxAToB.deltaCarbonKg + fluxBToA.deltaCarbonKg}`
    );
    assert.ok(
      Math.abs(fluxAToB.deltaMineralKg + fluxBToA.deltaMineralKg) < 1e-9,
      `Mineral conservation error: ${fluxAToB.deltaMineralKg + fluxBToA.deltaMineralKg}`
    );
  });
  ...
  it('INV-METRIC-ENTROPY: guarantees non-negative entropy generation for thermal diffusion', () => {
    ...
    const stateWarm: CellThermodynamicState = {
      waterMassKg: 50_000,
      carbonMassKg: 200,
      mineralMassKg: 50,
      dissolvedOxygenKg: 4,
    ...
    const stateCold: CellThermodynamicState = {
      waterMassKg: 50_000,
      carbonMassKg: 200,
      mineralMassKg: 50,
      dissolvedOxygenKg: 4,
    ...
    assert.ok(
      fluxWarmToCold.entropyProducedJPerK >= 0,
      `Warm->Cold entropy generation must be positive: ${fluxWarmToCold.entropyProducedJPerK}`
    );
    assert.ok(
      fluxColdToWarm.entropyProducedJPerK >= 0,
      `Cold->Warm entropy generation must be positive: ${fluxColdToWarm.entropyProducedJPerK}`
    );
    // Both frames compute identical positive physical entropy generation
    assert.ok(
      Math.abs(fluxWarmToCold.entropyProducedJPerK - fluxColdToWarm.entropyProducedJPerK) < 1e-9
    );
  });
});