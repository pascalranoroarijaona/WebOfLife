...
import { validateStateProperties, ValidationResult, ValidationFailure } from '../src/thermodynamics/state_validator.js';
...
describe('Sprint 033: Thermodynamic State Vector Property Validator Helper', () => {
  it('TC-01: Valid State Vector should pass with zero errors', () => {
    ...
    const res: ValidationResult = validateStateProperties(state);
  ...
  it('TC-02: Negative energy should fail with energy bound error', () => {
    ...
    assert.ok((res.errors as ValidationFailure[]).some(e => e.reason.includes("energy")));
  });
  ...
  it('TC-03: Negative entropy should fail with entropy bound error', () => {
    ...
    const res = validateStateProperties(state);
    assert.strictEqual(res.valid, false);
    assert.ok((res.errors as ValidationFailure[]).some(e => e.reason.includes("entropy")));
  });
  ...
  it('TC-04: Negative temperature should fail with absolute zero boundary error', () => {
    ...
    const res = validateStateProperties(state);
    assert.strictEqual(res.valid, false);
    assert.ok((res.errors as ValidationFailure[]).some(e => e.reason.includes("temperature")));
  });
  ...
  it('TC-05: Negative stock inventory should fail with non-negative stock error', () => {
    ...
    const res = validateStateProperties(state);
    assert.strictEqual(res.valid, false);
    assert.ok((res.errors as ValidationFailure[]).some(e => e.reason.includes("Stock inventory 'C'")));
  });
