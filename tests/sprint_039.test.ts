...
describe("Sprint 039 — Canonical H3 Regex Assertions & Error Topologies", () => {
    ...
    it("should accept valid Mode-1 canonical 15-character H3 tokens across resolutions 0-15", () => {
      for (const token of validIndices) {
        assert.doesNotThrow(
          ...
          `Expected valid token '${token}' to pass pattern assertion`
        );
  ...
  describe("2. Rejection of Syntactically Invalid Tokens", () => {
      ...
      for (const { token, reason } of invalidLengthCases) {
        assert.throws(
          ...
          (err: unknown) => {
            ...
            assert.strictEqual((err as H3ValidationError).token, token);
            return true;
          }
        );
    ...
    it("should reject non-string inputs with H3ValidationError preserving raw input token", () => {
      ...
      for (const input of nonStringInputs) {
        assert.throws(
          () => assertCanonicalH3Pattern(input as unknown as string),
          (err: unknown) => {
            assert.ok(err instanceof H3ValidationError);
            assert.strictEqual((err as H3ValidationError).token, input as unknown as string);
            assert.match((err as H3ValidationError).message, /Token must be a string/);
            return true;
          }
        );
  ...
  describe("3. Error Subtyping and Topology Invariants", () => {
    it("should conform to Error -> SpatialGridError -> H3ValidationError inheritance chain", () => {
      try {
        ...
        assert.strictEqual((err as H3ValidationError).token, "corrupted_index");
        assert.match(
          (err as H3ValidationError).message,
          /Invalid canonical H3 index token 'corrupted_index'/
        );
      }
    });
  });
  ...
  describe("4. H3Grid Operations & Resolution Parsing", () => {
    ...
    it("should guard H3Grid.getNeighbors with assertCanonicalH3Pattern", () => {
      ...
      assert.throws(
        () => H3Grid.getNeighbors("invalid_token"),
        (err: unknown) => err instanceof H3ValidationError
      );
    });
    ...
    it("should guard H3Grid.kRing with assertCanonicalH3Pattern and radius bounds", () => {
      ...
      assert.throws(
        () => H3Grid.kRing("8828308281fffff", -1),
        (err: unknown) => err instanceof SpatialGridError
      );

      assert.throws(
        () => H3Grid.kRing("invalid_token", 1),
        (err: unknown) => err instanceof H3ValidationError
      );
    });
  });
  ...
  describe("5. SpatialMonad Conservation Invariants", () => {
    const healthyStocks: ThermodynamicStocks = {
      carbon: 1000.0,
      water: 500.0,
      nitrogen: 50.0,
      phosphorus: 10.0,
    ...
    it("should reject SpatialMonad instantiation when token violates canonical pattern", () => {
      assert.throws(
        () => createSpatialMonad("8828308281ffff", healthyStocks),
        (err: unknown) => err instanceof H3ValidationError
      );
    });

    it("should reject SpatialMonad with negative stock maintaining mass conservation", () => {
      const leakingStocks: ThermodynamicStocks = {
        ...healthyStocks,
        carbon: -0.001,
      };

      assert.throws(
        () => createSpatialMonad("8828308281fffff", leakingStocks),
        (err: unknown) => {
          assert.ok(err instanceof SpatialGridError);
          assert.match(
            (err as SpatialGridError).message,
            /Non-physical negative stock detected/
          );
          return true;
        }
      );
