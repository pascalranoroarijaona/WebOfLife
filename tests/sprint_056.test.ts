...
describe("Sprint 056: assertValidCoordinatePair & Geodesic Boundary Guard", () => {
  describe("1. Nominal Geodesic Coordinates", () => {
    ...
    it("should accept composite object formats { lat, lon } and { latitude, longitude }", () => {
      ...
      assert.doesNotThrow(() =>
        assertValidCoordinatePair({ latitude: -12.5, longitude: 45.2 })
      );
      assert.strictEqual(isValidCoordinatePair({ lat: 10, lon: 20 }), true);
      assert.strictEqual(
        isValidCoordinatePair({ latitude: -12.5, longitude: 45.2 }),
        true
      );
    });
  });
  ...
  describe("2. Boundary Tolerance & Longitude Wrapping", () => {
    ...
    it("should allow longitude in [0, 360] range when allowNormalizedPositiveLon is enabled", () => {
      ...
      assert.doesNotThrow(() =>
        assertValidCoordinatePair(0, 350, { allowNormalizedPositiveLon: true })
      );

      // Still rejects > 360 + epsilon
      assert.throws(() =>
        assertValidCoordinatePair(0, 360.5, { allowNormalizedPositiveLon: true })
      );
    });
  });
  ...
  describe("3. IEEE 754 Sanity & Type Violations", () => {
    it("should reject NaN for latitude and longitude", () => {
      assert.throws(
        () => assertValidCoordinatePair(NaN, 0),
        CoordinateBoundaryError
      );
      assert.throws(
        () => assertValidCoordinatePair(0, NaN),
        CoordinateBoundaryError
      );
    ...
    it("should reject +/- Infinity", () => {
      assert.throws(
        () => assertValidCoordinatePair(Infinity, 0),
        CoordinateBoundaryError
      );
      assert.throws(
        () => assertValidCoordinatePair(0, -Infinity),
        CoordinateBoundaryError
      );
    ...
    it("should reject non-numeric and null/undefined values", () => {
      assert.throws(
        () => assertValidCoordinatePair(null as any, 0),
        CoordinateBoundaryError
      );
      assert.throws(
        () => assertValidCoordinatePair(undefined as any, 0),
        CoordinateBoundaryError
      );
      assert.throws(
        () => assertValidCoordinatePair("10" as any, 20),
        CoordinateBoundaryError
      );
      assert.throws(
        () => assertValidCoordinatePair({} as any),
        CoordinateBoundaryError
      );
    });
  });
  ...
  describe("5. H3AdjacencyService Gating & Calculations", () => {
    ...
    it("should reject invalid coordinates in distance calculation", () => {
      assert.throws(
        () => H3AdjacencyService.getGreatCircleDistance(91, 0, 0, 0),
        CoordinateBoundaryError
      );
      assert.throws(
        () => H3AdjacencyService.getGreatCircleDistance(0, 0, 0, -181),
        CoordinateBoundaryError
      );
    });
    ...
    it("should calculate bearings and reject invalid coordinates", () => {
      ...
      assert.throws(
        () => H3AdjacencyService.latLonToBearing(95, 0, 0, 0),
        CoordinateBoundaryError
      );
    });
    ...
    it("should find nearest neighbors with strict candidate coordinate validation", () => {
      ...
      assert.throws(
        () => H3AdjacencyService.findKNearestNeighbors(0, 0, corruptedCandidates, 2),
        CoordinateBoundaryError
      );
    });
  });
  ...
  describe("6. Thermodynamic Invariants & SpatialTransportMonad Conservation", () => {
    ...
    it("should abort transport and prevent thermodynamic leakage if coordinates are corrupted", () => {
      ...
      assert.throws(
        () => SpatialTransportMonad.of([nodeA, corruptedNode]),
        CoordinateBoundaryError
      );
    });
  });
