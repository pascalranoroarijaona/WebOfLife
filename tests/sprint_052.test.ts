...
import { StateValidator, ConservationReport } from '../src/thermodynamics/state_validator.js';
import { StateVector } from '../src/thermodynamics/types.js';
...
describe('Sprint 052: Thermodynamic State Vector Stock Conservation Asserter', () => {
  it('should validate closed-system mass conservation accurately', () => {
    ...
    const boundaryFluxes = new Map<string, number>([
  ...
  it('should detect thermodynamic conservation violations and throw error', () => {
    ...
    const boundaryFluxes = new Map<string, number>([['Energy', 10.0]]); // Expected delta: 10 * 1 = 10
  ...
  it('should handle open-system geochemical cycles and carbon balance invariant checks', () => {
    ...
    const carbonReport = reports.find((r: ConservationReport) => r.element === 'Carbon');
