...
import { ThermodynamicStateMonad, IThermodynamicStateVector, ThermodynamicStateVector } from '../src/thermodynamics/types.js';

describe('Sprint 009: Thermodynamic State Vector & Second Law Monad', () => {
  const baseVector: ThermodynamicStateVector = new ThermodynamicStateVector({
    timestamp: Date.now(),
  ...
  it('should reject initialization if entropy generation rate is negative', () => {
    const invalidVector: ThermodynamicStateVector = new ThermodynamicStateVector({
      ...baseVector.toObject(),
  ...
  it('should enforce Second Law ($\dot{S}_{\text{gen}} \ge 0$) across state transitions', () => {
    ...
    const validNext = monad.map((current: IThermodynamicStateVector) => {
    ...
    assert.throws(() => {
      monad.map((current: IThermodynamicStateVector) => new ThermodynamicStateVector({
        ...current,
